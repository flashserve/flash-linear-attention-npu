...
def _shape_table() -> list[dict]:
    ...
    # Packed varlen after the first T>=256 dense rows so ATK perf JSON
    # (first 6 profiles with T>=256 and no seqlens) stays bit-identical.
    add("varlen_g1_align", 1, 4, 4, 192, 128, seqlens=(64, 64, 64))
    add("varlen_g1_tail", 1, 4, 4, 224, 128, seqlens=(80, 48, 96))
    add("varlen_g2_v256", 1, 4, 8, 160, 256, seqlens=(96, 64))
    add("varlen_g3_mix", 1, 4, 12, 176, 128, seqlens=(32, 64, 80))
    add("varlen_g4_v256", 1, 2, 8, 192, 256, seqlens=(128, 64))
    add("varlen_near_chunk", 1, 8, 8, 192, 128, seqlens=(65, 127))
    add("varlen_many_short", 1, 4, 4, 256, 128, seqlens=(16, 16, 16, 16, 32, 32, 64, 64))
    add("varlen_g2_tail", 1, 8, 16, 140, 128, seqlens=(70, 70))
    ...
    # Dedup by shape + seqlens, keep first 100.
    seen = set()
    uniq = []
    for row in rows:
        key = (
...
def dump_json_files(out_dir: Path | None = None) -> None:
    ...
    mss_idx = [
        0,
        _first("tail_T160"),
        _first("partial_pack_HV6"),
        _first("r2_T64_V256"),
        _first("varlen_g1_tail"),
