...
case，并使用多进程任务模式，使 `-to` 真实约束当前 case，不把排队时间算入其他
case。精度和确定性单 case 上限为 60 秒；内存检查需要对全量模板聚合对象执行 host
...
由 watchdog 设置相同的 60/1000 秒硬截止；截止后先向整个进程组发送 TERM，
10 秒后仍有 worker 存活则发送 KILL。该 10 秒仅用于回收已经判定超时的进程，
不延长单条 case 的执行预算；一次执行多条 case 时不设置整批进程截止，避免把
整批累计耗时误判为单 case 超时。
...
正式矩阵的分片大小固定为 1；精度、确定性和性能超时参数只能设为 1 到 60 秒，
内存检查 `MSS_TIMEOUT` 只能设为 1 到 1000 秒且默认 1000 秒。正式确定性矩阵固定执行
50 轮。续跑时同时设置既有
...
| --- | --- | --- | --- | --- |
| A2 (`ascend910b`) | 200/200 Pass | 未完成 | 未完成 | 精度汇总已核验；单 key memcheck 烟测发现越界，修复后再执行完整矩阵 |
