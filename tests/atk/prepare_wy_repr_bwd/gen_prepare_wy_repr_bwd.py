...
SMALL_PROFILE_OVERRIDES = {
    # Avoid the degenerate two single-token sequences selected for this slot.
    # Keep the same tiling signature while increasing T and reducing heads so
    # the total input size remains close to one tenth of the DA reference.
    (3, 20): {"B": 1, "HK": 2, "HV": 4, "T": 17, "mean_len": 3},
}
...
EXPECTED_TILING_KEYS = {
    f"{dtype}_{gtype}_v{V}_c{chunk_size}"
...
def _tiling_key(profile):
    return (
        f"{profile['dtype']}_{profile['gtype']}_"
        f"v{profile['V']}_c{profile['chunk_size']}"
    )
...
def _shrink_profile(profile):
    ...
    elements_per_token = _input_elements({**profile, "T": 1})
    ...
    profile["input_elements"] = _input_elements(profile)
    profile["element_ratio_to_da"] = round(
        profile["input_elements"] / original_elements, 6
    )
    return profile
...
def _build_small_profiles():
                    ...
                    for V in SMALL_V_VALUES:
                        for chunk_size in CHUNK_SIZES:
                            for dtype, gtype in DTYPE_GTYPE_PAIRS:
                                ...
                                if _da_shape_elements(profile) <= DA_SMALL_ELEMENT_LIMIT:
    ...
    for bucket_index, (bucket_key, required_count) in enumerate(
        ...
        for profile in bucket:
            ...
            profile.update(
                SMALL_PROFILE_OVERRIDES.get((bucket_index, profile_index), {})
            )
            profile["tiling_key"] = _tiling_key(profile)
            profile["input_elements"] = _input_elements(profile)
            profile["element_ratio_to_da"] = round(
                profile["input_elements"] / profile["source_da_elements"], 6
            )
...
def _build_large_profiles(reference_cases):
    ...
    for profile_index, name in enumerate(LARGE_REFERENCE_NAMES):
        ...
        elements = _input_elements(profile)
        if elements > LARGE_ELEMENT_CAP:
            raise RuntimeError(
                f"large profile {name} exceeds input cap: {elements}"
            )
...
def _build_profiles():
    ...
    actual_tiling_keys = {profile["tiling_key"] for profile in profiles}
    if actual_tiling_keys != EXPECTED_TILING_KEYS:
        missing = sorted(EXPECTED_TILING_KEYS - actual_tiling_keys)
        raise RuntimeError(f"profiles do not cover all 24 tiling keys: {missing}")
...
if GENERATOR_REGISTRY is not None:
    ...
    @GENERATOR_REGISTRY.register("generator_prepare_wy_repr_bwd")
    class Generator(CaseGenerator):
        ...
        def after_case_config(self, case_config: CaseConfig) -> CaseConfig:
            ...
            for item in case_config.inputs:
                ...
                elif cfg.name == "varlen":
                    cfg.range_values = int(bool(spec.get("varlen", False)))
                elif cfg.name == "mean_len":
                    cfg.range_values = int(spec.get("mean_len", 1))
