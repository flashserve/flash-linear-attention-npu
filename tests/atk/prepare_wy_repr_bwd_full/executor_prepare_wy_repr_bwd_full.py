"""ATK executor for prepare_wy_repr_bwd_full.

The CPU golden is embedded from fla/ops/ascendc/gdn/chunk_gdn_bwd/prepare_wy_repr_bwd_full/test/test.py.
"""
...
def run_cpu(spec: dict[str, Any], high_precision: bool = False):
    return _compute_full_golden(build_inputs(spec, torch.device("cpu")), high_precision)


def run_npu(spec: dict[str, Any], input_data: InputDataset):
    inputs = build_inputs(spec, _marker_device(input_data))

    return ascendc.prepare_wy_repr_bwd_full(
        inputs["k"],
        inputs["v"],
        inputs["beta"],
        inputs["A"],
