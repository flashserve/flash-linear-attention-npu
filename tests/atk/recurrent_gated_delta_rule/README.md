...
`atk_recurrent_gated_delta_rule.json` 由 `gen_recurrent_gated_delta_rule.py` 确定性生成，固定包含 163 条正向用例。每条 `case_spec` 都携带详细设计 `design_id`，映射关系如下：
...
| 32-41 | RGDR-P033-P042 | 10 | `Dk/Dv=16..128` 的对齐边界、UB profile |
| 42-57 | RGDR-P043-P058 | 16 | `Nk/Nv=1..256`、GVA group size |
| 58-73 | RGDR-P059-P074 | 16 | state stride/layout、稀疏/逆序/重复 index |
| 74-89 | RGDR-P075-P090 | 16 | 平台、调用通路代表 shape、非连续输入、连续调用与回归 |
| 90-105 | RGDR-G001-G016 | 16 | GVA head 映射与 group size |
| 106-121 | RGDR-G017-G032 | 16 | GVA gate、state dtype、可追踪 head 数据 |
| 122-137 | RGDR-G033-G048 | 16 | GVA varlen、prefix、accepted 与稀疏 block |
| 138-146 | RGDR-G049-G057 | 9 | GVA 对齐维度边界、上界与 UB 分片（`Dk/Dv<=128`） |
| 147-162 | RGDR-G058-G073 | 16 | GVA state layout、index、非连续输入与复杂视图 |
...
正向 JSON 只包含 `Dk`、`Dv` 为 16 的整数倍的合法 shape；维度泛化分组覆盖范围为 `16..128`。算子原有支持范围仍为 `Dk/Dv<=512`，固定功能与平台回归用例可覆盖更大合法维度。非 16 倍数的维度属于非法输入，由 `fla_npu` wrapper 和 op_host tiling 共同拦截；对应拦截由 op_host UT 覆盖。
