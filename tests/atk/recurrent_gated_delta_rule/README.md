...
- `Nv` 必须是 `Nk` 的整数倍；`Nk <= 256`、`Nv <= 256`、`Dk <= 128`、`Dv <= 128`，且 `Dk`、`Dv` 必须是 16 的整数倍；每条有效序列长度不超过 8。
...
`atk_recurrent_gated_delta_rule.json` 由 `gen_recurrent_gated_delta_rule.py` 确定性生成，固定包含 140 条正向用例。每条 `case_spec` 都携带详细设计 `design_id`，映射关系如下：

| ATK Case ID | 详细设计 ID | 数量 | 覆盖范围 |
|---:|---|---:|---|
| 0-10 | RGDR-P001-P011 | 11 | gate 组合、BF16/FP32 state、A2/A3 归约路径 |
| 11-26 | RGDR-P012-P027 | 16 | 变长、prefix、零长度逻辑序列、accepted token |
| 27 | RGDR-P028 | 1 | `Dk=Dv=128` 的对齐维度、UB profile |
| 28-43 | RGDR-P029-P044 | 16 | `Nk/Nv=1..256`、GVA group size |
| 44-59 | RGDR-P045-P060 | 16 | state stride/layout、稀疏/逆序/重复 index |
| 60-74 | RGDR-P061-P075 | 15 | 平台、调用通路代表 shape、非连续输入、连续调用与回归 |
| 75-90 | RGDR-G001-G016 | 16 | GVA head 映射与 group size |
| 91-106 | RGDR-G017-G032 | 16 | GVA gate、state dtype、可追踪 head 数据 |
| 107-122 | RGDR-G033-G048 | 16 | GVA varlen、prefix、accepted 与稀疏 block |
| 123 | RGDR-G049 | 1 | GVA 对齐维度、UB 分片（`Dk=Dv=128`） |
| 124-139 | RGDR-G050-G065 | 16 | GVA state layout、index、非连续输入与复杂视图 |
...
正向 JSON 只包含 `Dk`、`Dv` 不超过 128 且为 16 的整数倍的合法 shape；维度泛化分组仅保留 `Dk=Dv=128`。超过 128 或非 16 倍数的维度属于非法输入，由 `fla_npu` wrapper 和 op_host tiling 共同拦截；对应拦截由 op_host UT 覆盖。
