...
正式验收统一使用 `tests/atk/run_test_cpu.sh`。`smoke_operator.py` 用于在进入 ATK
前直接调用完整算子，快速确认 ABI、kernel 启动和核间同步是否正常。
...
```bash
python tests/atk/chunk_gdn_bwd_intra/scripts/smoke_operator.py \
  --dtype bf16 --g-dtype fp32 --beta-dtype fp32 \
