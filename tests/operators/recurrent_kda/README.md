...
| `accuracy/run_generated_generalization.py` | 生成式泛化入口，覆盖 Head/sequence 轴、两档 K/V、有效 bool 组合和 launch-time 统计 |
...
覆盖 96 个有效 bool 组合和 192 个 layout/bool 交叉组合；`state_v_first=False` 等非法组合只进入负向契约。
