...
| `accuracy/run_generated_generalization.py` | 生成式泛化入口，覆盖 Head/sequence 轴、两档 K/V、非连续 state、有效 bool 组合和 launch-time 统计 |
...
A2/A3/A5 通过 `FLA_NPU_SOC` 选择。生成式泛化脚本固定 `K/V` 仅在 `128x128`、`128x256` 两档内取值，
并在部分 case 中构造非连续 initial_state view，覆盖 96 个有效 bool 组合和 192 个 layout/bool 交叉组合；`state_v_first=False`
等非法组合只进入负向契约。
Kimi H96/D128 smoke 仅检查 NPU launch、输出 shape 和抽样 finite，不跑 CPU golden。
