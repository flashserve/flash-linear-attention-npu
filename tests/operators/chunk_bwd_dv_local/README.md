# ChunkBwdDvLocal 测试归档

## 1. 唯一用例规格

`tests/op_cases/chunk_bwd_dv_local.json` 统一保存 shape、dtype、layout、属性、可选输入、SOC、运行通路、随机种子、
...
| `accuracy/test_chunk_bwd_dv_local.py` | `fla_npu.ops.ascendc` 主精度、泛化、边界和回归入口 |
| `routes/test_aclnn_chunk_bwd_dv_local.cpp` | aclnn 两段式接口签名、workspace/executor/stream 契约 |
| `routes/test_direct_chunk_bwd_dv_local.cpp` | host tiling 结果驱动的 `<<<>>>` 参数和 launch 契约 |
...
FLA_NPU_RUN_OPERATOR_TESTS=1 pytest -q tests/operators/chunk_bwd_dv_local/accuracy/test_chunk_bwd_dv_local.py
FLA_NPU_CASE_TAGS=generalization FLA_NPU_RUN_OPERATOR_TESTS=1 pytest -q tests/operators/chunk_bwd_dv_local/accuracy/test_chunk_bwd_dv_local.py
