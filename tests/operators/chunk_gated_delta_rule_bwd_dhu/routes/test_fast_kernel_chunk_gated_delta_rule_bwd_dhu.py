...
FIX_TEST_CONFIGS = [
    ...
    for case_id, values in legacy_param_values(
        "chunk_gated_delta_rule_bwd_dhu", "fast_kernel_launch", group="fixed", dtype_module=torch
    )
]
...
VARIABLE_TEST_CONFIGS = [
    pytest.param(*values, id=case_id)
    for case_id, values in legacy_param_values(
        "chunk_gated_delta_rule_bwd_dhu", "fast_kernel_launch", group="variable", dtype_module=torch
    )
]
