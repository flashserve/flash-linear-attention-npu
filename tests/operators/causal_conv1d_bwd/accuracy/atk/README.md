# causal_conv1d_bwd ATK 回归资产

## 用例来源

唯一用例来源是 tests/op_cases/causal_conv1d_bwd.json 中 legacy.suite=atk_regression 的 150 条记录。
...
## 物化与执行
    ...
    python3 -m tests.operators._shared.legacy_cases materialize \
      --op causal_conv1d_bwd \
      --suite atk_regression \
      --output /tmp/causal_conv1d_bwd_atk.json
