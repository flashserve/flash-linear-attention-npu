...
`tests/op_cases/kda_gate_cumsum.json` 统一保存 shape、dtype、layout、属性、可选输入、SOC、运行通路、随机种子、
...
| `accuracy/test_kda_gate_cumsum.py` | `fla_npu.ops.ascendc` 主精度、泛化、边界和回归入口 |
| `routes/test_aclnn_kda_gate_cumsum.cpp` | aclnn 两段式接口签名、workspace/executor/stream 契约 |
| `routes/test_direct_kda_gate_cumsum.cpp` | host tiling 结果驱动的 `<<<>>>` 参数和 launch 契约 |
...
FLA_NPU_RUN_OPERATOR_TESTS=1 pytest -q tests/operators/kda_gate_cumsum/accuracy/test_kda_gate_cumsum.py
FLA_NPU_CASE_TAGS=generalization FLA_NPU_RUN_OPERATOR_TESTS=1 pytest -q tests/operators/kda_gate_cumsum/accuracy/test_kda_gate_cumsum.py
pytest -q tests/operators/kda_gate_cumsum/ut
python tests/operators/kda_gate_cumsum/performance/profile.py --dry-run
FLA_NPU_RUN_OPERATOR_TESTS=1 pytest -q tests/operators/kda_gate_cumsum/st/test_example.py
```
