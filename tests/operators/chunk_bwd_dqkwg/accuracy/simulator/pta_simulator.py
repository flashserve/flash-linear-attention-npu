...
_REPO_ROOT = _pathlib.Path(__file__).resolve().parents[5]
...
if __name__ == "__main__":
    ...
    isVarLen = False
    chunk_size = 128
    cases = [list(values) for _, values in legacy_param_values(
        "chunk_bwd_dqkwg", "simulator_main", dtype_module=torch
    )]
    device_id = 5
    ...
    data_path = os.environ.get(
        "DQKWG_SIM_DATA_DIR",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "data"),
    )
    RUN_CPU = False
    SAVE_FILES = False
    if SAVE_FILES:
