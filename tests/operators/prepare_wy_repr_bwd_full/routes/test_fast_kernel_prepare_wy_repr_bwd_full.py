...
FIXED_CONFIGS = [
    ...
    for case_id, values in legacy_param_values(
        "prepare_wy_repr_bwd_full", "fast_kernel_launch", group="fixed", dtype_module=torch
    )
]
...
VARIABLE_CONFIGS = [
    pytest.param(*values, id=case_id)
    for case_id, values in legacy_param_values(
        "prepare_wy_repr_bwd_full", "fast_kernel_launch", group="variable", dtype_module=torch
    )
]
