...
def chunk_kda_fwd_kernel_intra_token_parallel(
    ...
    i_b, i_hg = tl.program_id(0), tl.program_id(1)
    ...
    for i_t in range(T_local):
        ...
        for j in range(i_ts, min(i_t + 1, min(T_local, i_ts + BC))):
...
def chunk_kda_fwd_intra_token_parallel(
    ...
    """
    Token-parallel implementation: each batch gets its own thread block.
