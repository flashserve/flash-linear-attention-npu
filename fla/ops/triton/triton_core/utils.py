...
# ============================================================================
# Environment check (runs once at import time)
# ============================================================================
...
# NPU takes priority; if not NPU, fall back to GPU detection
if _IS_NPU:
    device = 'npu'
...
# On NPU, GPU-specific features are not available and are hardcoded to safe defaults.
# On GPU, these are determined at runtime.
IS_INTEL_ALCHEMIST = False
IS_NVIDIA_HOPPER = (
    IS_NVIDIA and (
        'NVIDIA H' in torch.cuda.get_device_name(0) or torch.cuda.get_device_capability()[0] >= 9
    )
) if not _IS_NPU else False
IS_NVIDIA_BLACKWELL = False
USE_CUDA_GRAPH = False
IS_TF32_SUPPORTED = (IS_NVIDIA and torch.cuda.get_device_capability(0)[0] >= 8) if not _IS_NPU else False
IS_GATHER_SUPPORTED = hasattr(triton.language, 'gather')
IS_TMA_SUPPORTED = (IS_NVIDIA and torch.cuda.get_device_capability(0)[0] >= 9
                    and os.environ.get('FLA_NO_USE_TMA', '0') != '1'
                    and (hasattr(triton.language, '_experimental_make_tensor_descriptor')
                         or hasattr(triton.language, 'make_tensor_descriptor'))
                    ) if not _IS_NPU else False

# TF32 workaround for old NVIDIA cards (not applicable on NPU)
if not _IS_NPU and IS_NVIDIA and not IS_TF32_SUPPORTED:
    # Make old card happy, since triton will use tf32 by default.
    # This is a workaround for old nvidia card.
    os.environ['TRITON_F32_DEFAULT'] = 'ieee'

# Preserve lowercase aliases for backward compatibility
is_amd = IS_AMD
is_nvidia = IS_NVIDIA
...
# ============================================================================
# input_guard (with no_guard_contiguous enhancement + bug fix)
# ============================================================================
...
# ============================================================================
# NPU autotune config (for NPU Triton backends)
# ============================================================================
...
# ============================================================================
# Register lowercase aliases for all platform flags
# ============================================================================
