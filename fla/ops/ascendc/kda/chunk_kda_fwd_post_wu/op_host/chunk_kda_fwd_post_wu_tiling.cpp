...
#include "chunk_kda_fwd_post_wu_tiling.h"
...
constexpr size_t INPUT_U_SEED_IDX = 4;
...
ge::graphStatus Tiling4ChunkKdaFwdPostWu(gert::TilingContext *context)
{
    ...
    const int64_t totalChunks = hasCuSeqlens
        ? (chunkMetadata == nullptr ? 0 : chunkMetadata->GetStorageShape().GetShapeSize() / 4)
...
ge::graphStatus TilingPrepare4ChunkKdaFwdPostWu(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ChunkKdaFwdPostWu)
    .Tiling(Tiling4ChunkKdaFwdPostWu)
    .TilingParse<ChunkKdaFwdPostWuCompileInfo>(TilingPrepare4ChunkKdaFwdPostWu);
