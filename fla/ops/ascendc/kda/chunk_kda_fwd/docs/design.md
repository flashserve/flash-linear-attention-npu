...
`ChunkKdaFwdKernel<KdaPhase::PREPARE, SAFE_GATE, ...>`、`POST_WU` 和 `OUTPUT` 实例。PREPARE 在
kernel 入口读取一次 `safeGate`，选择 `<true>` 或 `<false>` 模板，chunk 热路径没有 runtime gate 分支。
key 只绑定内部语义阶段与 mixed AIC:AIV task 类型，不编码 safe gate、B/H/T、layout 或 shape 组合，
也不是公共接口属性。
...
不含 runtime dtype/phase 字段，并确认三个 KDA phase 与 safe gate 都实例化为编译期模板。精度用例
