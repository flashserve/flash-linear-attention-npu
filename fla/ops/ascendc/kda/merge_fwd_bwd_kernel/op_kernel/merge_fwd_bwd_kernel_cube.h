...
public:
    ...
    // AtlasA2 L0C is 128 KiB. Each 64x128 FP32 C tile is 32 KiB; 64 KiB
    // stride keeps C0/C1 disjoint without the 950 128 KiB split.
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t h = hBegin; h < hEnd; ++h) {
            ...
            for (int64_t step = 0; step < gemmSteps; ++step) {
                AicWaitChunkReady<PIPE_MTE2>(static_cast<uint16_t>(slot));
                LoadStageToL1(
                ...
                AicSetChunkFree<PIPE_FIX>(static_cast<uint16_t>(slot));
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>(kEventL0C0);
                GemmKSplitFixpipe(
                    copyL1ToL0A, copyL1ToL0B, tileMmad, l0A, l0B, tensorL1A1, tensorL1B,
                    tensorL0C1, h, 1, true, kEventL0C1);
                AicSetChunkC1Done<PIPE_FIX>(static_cast<uint16_t>(slot));
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>(kEventL0C1);
            }
        }
