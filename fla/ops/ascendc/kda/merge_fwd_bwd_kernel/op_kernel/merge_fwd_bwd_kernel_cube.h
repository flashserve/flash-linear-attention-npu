 ...
 *
 * 910b / 910_93 Mix Cube (dav-2201 / AtlasA2), host-split.
 * Same math as Ascend950: one rank-step GEMM on the shared FP32 scratch tensor.
 */
...
public:
    ...
    static constexpr uint32_t kL0CTileBytes = 64 * 1024;
    ...
    __aicore__ inline void Process()
    {
        ...
        AscendC::LocalTensor<CubeT> hL1 = resource.l1Buf.template GetBufferByByte<CubeT>(kL1HOffset);
        AscendC::LocalTensor<CubeT> a0L1 = resource.l1Buf.template GetBufferByByte<CubeT>(kL1A0Offset);
        AscendC::LocalTensor<CubeT> a1L1 = resource.l1Buf.template GetBufferByByte<CubeT>(kL1A1Offset);
        ...
        for (int64_t step = 1; step < n_; ++step) {
            ...
            for (uint64_t h = hBegin; h < hEnd; ++h) {
                CubeWaitVc(h);
                LoadStageToL1(tensorL1B, tensorL1A0, tensorL1A1, layoutH, layoutMTile, h, mBank);
                MmadKSplit(
                    copyL1ToL0A, copyL1ToL0B, tileMmad, l0A, l0B, tensorL1A0, tensorL1B, tensorL0C0,
                    false, kEventL0C0);
                DrainCopyC(tensorL0C0, h, 0, kEventL0C0);
                CubeSetCv0(h);
                MmadKSplit(
                    copyL1ToL0A, copyL1ToL0B, tileMmad, l0A, l0B, tensorL1A1, tensorL1B, tensorL0C1,
                    true, kEventL0C1);
...
private:
    ...
    __aicore__ inline void LoadStageToL1(
        ...
        LayoutH const &layoutH, LayoutM const &layoutM, uint64_t h, uint32_t mBank)
    {
        AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(kEventL1);
        ...
        TensorL0C const &tensorL0C, uint64_t h, int64_t tile, int32_t l0cEvent)
    {
        ...
        CopyCToScratchH(h, tensorL0C, tile);
    ...
    template <typename TensorL0C>
    __aicore__ inline void CopyCToScratchH(uint64_t h, TensorL0C const &tensorL0C, int64_t tile)
    {
        AscendC::GlobalTensor<float> gmC;
        gmC.SetGlobalBuffer(
            (__gm__ float *)scratch_ + ScratchHElemOff(h) +
        ...
        AscendC::GlobalTensor<OutT> const &gmC, TensorL0C const &tensorL0C, uint32_t dstStride)
    {
        ...
        params.quantPre = Gemm::Tile::CopyL0CToGmQuantMode<
        ...
        AscendC::Fixpipe<OutT, AccT, AscendC::CFG_ROW_MAJOR>(
            gmC[0], tensorL0C.data()[srcOffset], params);
    }
