...
ge::graphStatus Tiling4MergeFwdBwdKernel(gert::TilingContext *context)
{
    ...
    // Prefer the live platform count. compileInfo 28/56 is a 950 stub used only
    // when GetCoreNumAic returns the 2-core placeholder; max() would pin 910_93
    // (typically 20 AIC) to 28.
    ...
    // task_per_aic = min(ceil(HV / aicNum), 4); usedAic = ceil(HV / task_per_aic),
    // capped at physical AIC. Each Mix core pipelines up to 4 heads per round.
    const int64_t aicI64 = static_cast<int64_t>(std::max(1U, aicNum));
    const int64_t spread = (Hv + aicI64 - 1) / aicI64;
    const int64_t taskPerAic = std::max<int64_t>(
        1, std::min(spread, static_cast<int64_t>(MergeFwBwd::kMaxTaskPerAic)));
    uint32_t usedAic = static_cast<uint32_t>(
        std::max<int64_t>(1, (Hv + taskPerAic - 1) / taskPerAic));
    if (usedAic > aicNum) {
        usedAic = aicNum;
    }
