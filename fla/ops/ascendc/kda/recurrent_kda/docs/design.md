...
- 支持 `BSND` 和 `TND`，支持 dense 与 `cu_seqlens` 变长序列。
...
| State layout | `state_v_first=True`，`[seq_num,H_v,V,K]` |
...
Tiling 按逻辑序列和 value head 拆分任务。dense 模式未传 `cu_seqlens` 时，`seq_num=B`、每条序列长度为 `T`；
变长模式下，`seq_num=len(cu_seqlens)-1`，每段长度来自相邻 cumulative seqlens。当前单任务面向 recurrent 小步长，
每段序列长度限制为 `<=8`。
...
| `t` / `seqLen` | `uint32_t` | token 总数与单段最大长度 |
| `nk` / `dk` | `uint32_t` | query/key head 数与 K 维 |
| `nv` / `dv` | `uint32_t` | value head 数与 V 维 |
| `sBlockNum` / `b` | `uint32_t` | 状态序列数与 dense batch |
...
5. 计算 `S @ q` 写回 `out`，并在需要的位置更新 `final_state`。

### 7.2 内存规划

| 内存层级 | Buffer | 生命周期 | 用途 |
| --- | --- | --- | --- |
| GM | 输入/输出 tensor | kernel 全程 | q/k/v/g/beta/state/out/final_state |
...
算子为 AIV-only recurrent 小步长 kernel，单任务内部按 token 串行维护状态依赖。不存在跨 core 共享同一状态槽的写依赖；
...
- `cu_seqlens` 必须从 0 开始、非递减、末项等于 token 总数。
- 每段长度为 0 的序列不产生 token 输出，但状态 shape 仍保留对应 slot。
- BSND 变长要求物理 `B=1`，避免 dense batch 与 packed token 语义混用。
- `num_accepted_tokens` 只在提供 `ssm_state_indices` 时有效，用于定位 MTP decode 的状态槽。
