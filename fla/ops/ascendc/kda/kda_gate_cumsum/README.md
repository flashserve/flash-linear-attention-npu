...
| 模式 | dense/varlen、step-gate/safe raw-gate、四种 layout、整块/尾块 |

varlen 模式中，`cu_seqlens[0]` 必须为 0、末项等于 `T` 且序列非递减。fixed 与 varlen、尾块与整块遵循同一数学定义。
...
- 用例规格：`tests/op_cases/kda_gate_cumsum.json`；覆盖 dense/varlen、step-gate/safe raw-gate、四种 layout、整块/尾块。
