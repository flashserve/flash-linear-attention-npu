...
- README 所列 `dense/varlen、step-gate/safe raw-gate、四种 layout、整块/尾块` 场景精度与仓内参考实现一致。
...
实现类型：`ascendc`。Dtype：g 为 FP16/BF16/FP32；A_log/dt_bias/gk 为 FP32；cu_seqlens 为 INT64。Layout：rank4 使用 BSND/BNSD，rank3 使用 TND/NTD；layout 必须显式且不根据 shape 推导。模式：dense/varlen、step-gate/safe raw-gate、四种 layout、整块/尾块。
...
dense 按 (B,H_v,chunk) 分配，varlen 按 (sequence,H_v) 分配并在 core 内遍历该序列真实 chunk；每个 task 的 K 行按最多 256 元素的 UB 向量处理。
