 ...
 */
#include "merge_fwd_bwd_tiling.h"
#include "../../op_kernel/merge_fwd_bwd_struct.h"
#include "../../op_kernel/merge_fwd_bwd_tiling_key.h"
...
{
    auto *tiling = context->GetTilingData<MergeFwBwd::MergeFwdBwdTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);

    const auto *agDesc = context->GetInputDesc(0);
    const auto *agShapePtr = context->GetInputShape(0);
    ...
    const auto *compileInfo = context->GetCompileInfo<MergeFwdBwdCompileInfo>();
    ...
    // Head-parallel Mix 1:2. Mode 4 Ready/Free are per mix block, same as
    // ChunkKdaBwdRecompute. C scratch is placed after usedAic * kCoreStride.
    const uint32_t usedAic = static_cast<uint32_t>(
        std::max<int64_t>(1, std::min(static_cast<int64_t>(aicNum), Hv)));
    ...
    const uint64_t tilingKey = GET_TPL_TILING_KEY(
        dtype == ge::DT_FLOAT ? MERGE_FWD_BWD_TPL_FP32 : MERGE_FWD_BWD_TPL_BF16);
    ...
    const uint64_t userWs = static_cast<uint64_t>(usedAic) * MergeFwBwd::kCoreStride;
    const uint64_t cScratch = static_cast<uint64_t>(std::max<int64_t>(1, Hv)) *
                              static_cast<uint64_t>(MergeFwBwd::kCTilesPerHead) *
                              static_cast<uint64_t>(MergeFwBwd::kC0TileBytes);
    currentWorkspace[0] = sysWs + userWs + cScratch;

    auto *rawTiling = context->GetRawTilingData();
    OP_CHECK_NULL_WITH_CONTEXT(context, rawTiling);
    rawTiling->SetDataSize(sizeof(MergeFwBwd::MergeFwdBwdTilingData));
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingParse4MergeFwdBwd(gert::TilingParseContext *context)
{
    auto *compileInfo = context->GetCompiledInfo<MergeFwdBwdCompileInfo>();
...
IMPL_OP_OPTILING(MergeFwdBwd)
    .Tiling(Tiling4MergeFwdBwd)
    .TilingParse<MergeFwdBwdCompileInfo>(TilingParse4MergeFwdBwd);
