...
constexpr size_t INPUT_INITIAL_IDX = 14;
constexpr size_t INPUT_DHT_IDX = 15;
constexpr size_t INPUT_CU_SEQLENS_IDX = 16;
constexpr size_t ATTR_SCALE_IDX = 0;
constexpr size_t ATTR_CHUNK_SIZE_IDX = 1;
constexpr size_t ATTR_TOTAL_CHUNKS_IDX = 2;
...
ge::graphStatus Tiling4ChunkKdaBwd(gert::TilingContext *context)
{
    ...
    uint32_t blockDim = static_cast<uint32_t>(std::min<int64_t>(seqNum * qShape.GetDim(DIM_H), coreNum));
    ...
    tiling.set_hasDht(context->GetOptionalInputTensor(INPUT_DHT_IDX) != nullptr);
