...
1. 本 README 是 GDN Shape 符号的唯一维护位置，符号表版本为 `gdn-shape-v1`。
2. 每个算子的 `README.md`、`docs/design.md` 和 `docs/api.md` 直接链接本表，不复制本表或维护“本算子使用的符号”副本。
3. 固定 Shape 约束写入算子“已知限制”；枚举或离散属性的全部合法值写入属性表“取值范围”。
4. JSON 的 `shape` 字段必须使用本表名称；卷积等辅助算子可使用本文定义的 `D/W/L_s`。
5. 修改符号名称或语义时，必须提高符号表版本并同步所有受影响公式、API Shape、JSON case 和测试代码。
