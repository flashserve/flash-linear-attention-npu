...
class ChunkRecomputeWUFwdHO : public OpDef {
public:
    explicit ChunkRecomputeWUFwdHO(const char *name) : OpDef(name)
    {
        ...
        this->Input("beta").ParamType(REQUIRED).DataType(gateTypes).Format(formats)
            .UnknownShapeFormat(formats).AutoContiguous();
        this->Input("A").ParamType(REQUIRED).DataType(inputTypes).Format(formats)
            .UnknownShapeFormat(formats).AutoContiguous();
