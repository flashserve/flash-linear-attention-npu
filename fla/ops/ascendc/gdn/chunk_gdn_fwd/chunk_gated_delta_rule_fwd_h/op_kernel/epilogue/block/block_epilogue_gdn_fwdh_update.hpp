...
public:
    ...
    )
    {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        //AscendC::printf("ljm kgated invalid test");
        if constexpr (kGated) {
            //AscendC::printf("ljm kgated valid uupdate");
            AscendC::GlobalTensor<GElementInput> gkLastInput = gkInput[(chunkSize - 1) * kHeadDim + mOffset];
            ...
            if constexpr(std::is_same<GElementInput, float>::value) {
                AscendC::DataCopy(gkLastUbTensor, gkLastInput, mActualThisSubBlock); // 空间是否够用？
            } else {
