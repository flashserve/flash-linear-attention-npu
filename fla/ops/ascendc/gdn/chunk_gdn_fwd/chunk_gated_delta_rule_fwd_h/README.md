...
| `gkOptional` | 输入 | 接口为可选；`gk=None` 时走零开销路径 | Key-wise 逐通道门控衰减张量 | `FLOAT16`、`BFLOAT16`、`FLOAT` | `ND` | `[B, HV, T, K]` | 支持 |
...
- `gkOptional`（若提供）的形状必须为 `[B, HV, T, K]`，head 维与 `u` 对齐，`K` 维与 `w` 对齐；dtype 必须与 `gOptional` 一致。
...
- `gkOptional`：
  - 接口层为可选；`gk=None` 时走零开销路径（不分配 `kDecayWorkspace`，C2 直接读取原始 `k`）
  - 若提供，则形状必须为 `[B, HV, T, K]`，dtype 必须与 `gOptional` 一致
  - `gk=0` 时结果与 `gk=None` 完全等价
- `initalStateOptional`：
...
- `u, vNewOut`: `[B, HV, T, V]`
- `gOptional`: `[B, HV, T]`
- `gkOptional`: `[B, HV, T, K]`（若提供）；dtype 须与 `gOptional` 一致
...
```text
k_decay = k_{[c]} * E(gk_{[c]}[L-1, :] - gk_{[c]})
S_decay = E(g_{[c]}[L-1]) * E(gk_{[c]}[L-1, :])[:, None] * S_{[c]}
S_{[c+1]} = S_decay + k_decay^T @ v_decay
```

其中：
- `gk_{[c]}` 为第 `c` 个分块的 Key-wise 逐通道门控衰减值，形状 `[BT, K]`
- `k_decay` 为经 `gk` 衰减后的 Key，替代原始 `k` 参与 C2 状态更新
- `S_decay` 同时受 `g` 和 `gk` 衰减影响
- `gk=None` 时退化为仅 `g` 衰减路径（零开销），`gk=0` 时结果与 `gk=None` 完全等价
...
USE_DW = True
USE_G_GAMMA = False
USE_GK = True (optional, gk=None disables)
```
...
h, v_new, final_state = torch.ops.npu.npu_chunk_gated_delta_rule_fwd_h(
    k, w, u,
    g=...,                       # 当前不可为 None
    gk=None,                     # 可选；gk=None 时走零开销路径，gk 有值时形状 [B, HV, T, K]，dtype 与 g 一致
