...
| `g` | 必选 | `[B,T,Hv]` 或 `[T,Hv]`；FP32 | 门控值，固定为 sequence-major |
...
四种 QKV 布局分别为 `BNSD=[B,H,T,D]`、`BSND=[B,T,H,D]`、`NTD=[H,T,D]`、
`TND=[T,H,D]`。`Hv` 必须能被 `Hk` 整除。变长模式使用物理 `B=1`，`cuSeqlensOptional` 必须从 0 开始、
以 `T` 结束且单调不降。
...
| 名称 | 必选性 | Shape/Dtype | 说明 |
| --- | --- | --- | --- |
| `oOut` | 必选 | rank-4 为 `[B,T,Hv,V]`（BSND），rank-3 为 `[T,Hv,V]`（TND）；与 q 同 dtype | 前向输出固定使用 sequence-major 布局 |
...
| `aOutOptional` | 可选 | `[B,Hv,T,chunkSize]` 或 `[Hv,T,chunkSize]`；与 q 同 dtype | 系数矩阵；为空时使用内部临时张量 |
...
| 名称 | 当前支持范围 | 说明 |
| --- | --- | --- |
| `layout` | A5 `useExp2=true` 支持 `BNSD/BSND/NTD/TND`；其他路径仅 `BNSD` | q/k/v 的输入布局；BSND/TND 输入在拼接路径内转为 head-first，o 固定输出 BSND/TND |
