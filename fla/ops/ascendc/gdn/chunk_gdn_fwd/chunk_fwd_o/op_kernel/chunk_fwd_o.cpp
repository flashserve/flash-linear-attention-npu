                                           ...
                                           GM_ADDR userWorkspace, const ChunkFwdOTilingData *tilingData)
{
    ...
    gdnFwdO.Init(q, k, v, h, g, cuSeqlens, chunkIndices, o, tilingData, userWorkspace);
    gdnFwdO.Process();
}
                                         ...
                                         GM_ADDR userWorkspace, const ChunkFwdOTilingData *tilingData)
{
    ...
    if (tilingData->dataType == 1) {
        if (tilingData->gDataType == 2) {
            ChunkFwdOKernelImpl<bfloat16_t, float, WorkspaceT>(q, k, v, h, g, cuSeqlens, chunkIndices, o,
                                                               userWorkspace, tilingData);
        } else {
            ChunkFwdOKernelImpl<bfloat16_t, bfloat16_t, WorkspaceT>(q, k, v, h, g, cuSeqlens, chunkIndices, o,
                                                                    userWorkspace, tilingData);
        }
    } else {
        if (tilingData->gDataType == 2) {
            ChunkFwdOKernelImpl<half, float, WorkspaceT>(q, k, v, h, g, cuSeqlens, chunkIndices, o, userWorkspace,
                                                         tilingData);
        } else {
            ChunkFwdOKernelImpl<half, half, WorkspaceT>(q, k, v, h, g, cuSeqlens, chunkIndices, o, userWorkspace,
                                                        tilingData);
        }
    }
}
                                                   ...
                                                   GM_ADDR o, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    GDN::ChunkFwdODispatch(q, k, v, h, g, cu_seqlens, chunk_indices, o, user, &tilingData);
}
#endif
