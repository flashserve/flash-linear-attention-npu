...
| 模式 | 定长/变长序列、GVA、整块/尾块；g 为必选标量 gate |
...
- 用例规格：`tests/op_cases/chunk_fwd_o.json`；覆盖定长/变长序列、GVA、整块/尾块；g 为必选标量 gate。
- 参考实现：`torch_chunk_fwd_o_reference`；容差按 JSON 中各 dtype 的 `rtol/atol` 执行。该算子包含多段 Cube、Exp 和中间 dtype
  转换，`2e-2` 标准沿用仓内 direct-launch 数值基线；不得为规避失败继续放宽阈值或收窄输入范围。
