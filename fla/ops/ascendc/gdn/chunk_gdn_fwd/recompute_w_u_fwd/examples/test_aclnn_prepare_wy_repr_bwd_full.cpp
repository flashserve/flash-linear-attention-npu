...
/*!
 * \file test_aclnn_recompute_w_u_fwd.cpp
 * \brief
 */
// testci
...
int main()
{
    ...
    // 调用aclnnRecomputeWUFwd第一段接口
    ret = aclnnRecomputeWUFwdGetWorkspaceSize(k, v, beta, A, dA, dw, du, g, nullptr, nullptr, 64, dk, dv, dbeta,
                                                    dg, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnRecomputeWUFwdGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    // 调用aclnnRecomputeWUFwd第二段接口
    ret = aclnnRecomputeWUFwd(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnRecomputeWUFwd failed. ERROR: %d\n", ret); return ret);
