# ChunkGatedDeltaRuleFwdO 算子说明
`ChunkGatedDeltaRuleFwdO` 是一个用于分块门控 delta 规则（Chunk Gated Delta Rule）前向传播过程中的自定义算子。该算子根据 Query、Key、Value、Gate 以及前向隐藏状态，计算并输出前向注意力输出张量 `o`。
...
1. **获取 workspace 与执行器**
   调用 `aclnnChunkGatedDeltaRuleFwdOGetWorkspaceSize` 接口，获取算子执行所需的 workspace 大小，并创建执行器（executor）。

2. **执行算子计算**
   调用 `aclnnChunkGatedDeltaRuleFwdO` 接口，在指定的 workspace 和执行器下完成计算。

对应以下 C++ 接口（见 `op_host/op_api/aclnn_chunk_gated_delta_rule_fwd_o.h`）：

```cpp
/* funtion: aclnnChunkGatedDeltaRuleFwdOGetWorkspaceSize
 * parameters :
 * q : required
 * k : required
 * v : required
...
/* funtion: aclnnChunkGatedDeltaRuleFwdO
 * parameters :
 ...
 */
__attribute__((visibility("default")))
aclnnStatus aclnnChunkGatedDeltaRuleFwdO(
    void *workspace,
    uint64_t workspaceSize,
    aclOpExecutor *executor,
    aclrtStream stream);
...
def test_chunk_gated_delta_rule_fwd_o_fixed_len():
    # 参数设置
    ...
    o = ascendc_ops.npu_chunk_gated_delta_rule_fwd_o(
        q, k, v, h, scale,
        g=g,
        g_gamma=None,
        cu_seqlens=None,
...
if __name__ == "__main__":
    test_chunk_gated_delta_rule_fwd_o_fixed_len()
```
...
def test_chunk_gated_delta_rule_fwd_o_varlen():
    # 参数设置：变长模式仅支持 B = 1
...
if __name__ == "__main__":
    test_chunk_gated_delta_rule_fwd_o_varlen()
```
...
├── op_host/
│   ├── CMakeLists.txt
│   ├── chunk_gated_delta_rule_fwd_o_def.cpp
│   ├── chunk_gated_delta_rule_fwd_o_tiling.cpp
│   ├── chunk_gated_delta_rule_fwd_o_tiling.h
│   ├── chunk_gated_delta_rule_fwd_o_tiling_processor.h
│   └── op_api/
│       ├── aclnn_chunk_gated_delta_rule_fwd_o.cpp
│       ├── aclnn_chunk_gated_delta_rule_fwd_o.h
│       ├── chunk_gated_delta_rule_fwd_o.cpp
│       └── chunk_gated_delta_rule_fwd_o.h
├── op_kernel/
│   ├── chunk_gated_delta_rule_fwd_o.cpp
│   ├── chunk_gated_delta_rule_fwd_o_struct.h
│   ├── epilogue/
│   └── gemm/
└── tests/
    └── pta/
