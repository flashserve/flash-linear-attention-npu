...
namespace l0op {
OP_TYPE_REGISTER(ChunkGatedDeltaRuleFwdO);

const std::array<const aclTensor *, 1> ChunkGatedDeltaRuleFwdO(
    const aclTensor *q,
    const aclTensor *k,
    const aclTensor *v,
    const aclTensor *h,
...
{
    L0_DFX(ChunkGatedDeltaRuleFwdO, q, k, v, h, g, cuSeqlensOptional, chunkOffsetsOptional, scale, chunkSize, useExp2, stateVFirst,
           outputLayout, oOut);
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ChunkGatedDeltaRuleFwdO,
