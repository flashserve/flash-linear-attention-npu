 ...
 */
#include "aclnn_chunk_gated_delta_rule_fwd_o.h"
#include "chunk_gated_delta_rule_fwd_o.h"
#include <dlfcn.h>
#include <new>
...
struct ChunkGatedDeltaRuleFwdOParams {
...
static aclnnStatus CheckNotNull(ChunkGatedDeltaRuleFwdOParams params)
{
...
static aclnnStatus CheckFormat(ChunkGatedDeltaRuleFwdOParams params)
{
    return ACLNN_SUCCESS;
}

static aclnnStatus CheckShape(ChunkGatedDeltaRuleFwdOParams params)
{
    const auto &vShape = params.v->GetViewShape();
    CHECK_COND(vShape.GetDimNum() == CHUNK_GATED_DELTA_RULE_FWD_O_QKV_DIM_NUM, ACLNN_ERR_PARAM_INVALID,
               "v should be 4D [B, HV, T, V].");
    CHECK_COND(vShape.GetDim(CHUNK_GATED_DELTA_RULE_FWD_O_DIM_HEAD_DIM) <= CHUNK_GATED_DELTA_RULE_FWD_O_MAX_V_HEAD_DIM, ACLNN_ERR_PARAM_INVALID,
...
static aclnnStatus ParamsDataContiguous(ChunkGatedDeltaRuleFwdOParams &params, aclOpExecutor *executorPtr)
{
...
static aclnnStatus CheckDtype(ChunkGatedDeltaRuleFwdOParams params)
{
    return ACLNN_SUCCESS;
}

static aclnnStatus CheckParams(ChunkGatedDeltaRuleFwdOParams params)
{
...
aclnnStatus aclnnChunkGatedDeltaRuleFwdOGetWorkspaceSize(
    const aclTensor *q,
    const aclTensor *k,
    const aclTensor *v,
    const aclTensor *h,
...
{
    ChunkGatedDeltaRuleFwdOParams params{q, k, v, h, g, cuSeqlensOptional, chunkOffsetsOptional, scale, chunkSize,
                           useExp2, stateVFirst, outputLayout, oOut};
    // Standard syntax, Check parameters.
    L2_DFX_PHASE_1(aclnnChunkGatedDeltaRuleFwdO,
    ...
    auto result = l0op::ChunkGatedDeltaRuleFwdO(params.q, params.k, params.v, params.h, params.g, params.cuSeqlensOptional,
...
aclnnStatus aclnnChunkGatedDeltaRuleFwdO(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnChunkGatedDeltaRuleFwdO);
    CHECK_COND(CommonOpExecutorRun(workspace, workspaceSize, executor, stream) == ACLNN_SUCCESS, ACLNN_ERR_INNER,
               "This is an error in ChunkGatedDeltaRuleFwdO launch aicore.");
    return ACLNN_SUCCESS;
}
