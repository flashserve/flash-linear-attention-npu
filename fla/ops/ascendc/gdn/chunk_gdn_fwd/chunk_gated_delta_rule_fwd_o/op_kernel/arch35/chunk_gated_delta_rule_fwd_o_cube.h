...
#ifndef CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_CUBE_H
#define CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_CUBE_H
...
class ChunkGatedDeltaRuleFwdOA5CubeProcess {
public:
    ...
    static constexpr uint32_t kBt = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
    static constexpr uint32_t kK = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_K);
    static constexpr uint32_t kV = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_V);
    static constexpr uint32_t kL0BufferCount = CHUNK_GATED_DELTA_RULE_FWD_O_L0_BUFFER_COUNT;
    static constexpr uint32_t kL1ResidentHeadCount = CHUNK_GATED_DELTA_RULE_FWD_O_L1_RESIDENT_HEAD_COUNT;
    static constexpr uint32_t kL1EventCount = 2U * kL1ResidentHeadCount;
    static constexpr TEventID kL1EventBase = 5;

    static_assert(CHUNK_GATED_DELTA_RULE_FWD_O_L0_A_BYTES * kL0BufferCount <= ArchTag::L0A_SIZE,
                  "Stage2 L0A ping/pong exceeds architecture limit.");
    static_assert(CHUNK_GATED_DELTA_RULE_FWD_O_L0_B_BYTES * kL0BufferCount <= ArchTag::L0B_SIZE,
                  "Stage2 L0B ping/pong exceeds architecture limit.");
    static_assert(CHUNK_GATED_DELTA_RULE_FWD_O_L0_C_BYTES * kL0BufferCount <= ArchTag::L0C_SIZE,
                  "Stage2 L0C ping/pong exceeds architecture limit.");
    static_assert(CHUNK_GATED_DELTA_RULE_FWD_O_L1_STAGE2_END <= CHUNK_GATED_DELTA_RULE_FWD_O_L1_APRIME_BASE,
                  "Stage2 L1 stream slots overlap Stage4 resident buffers.");
    static_assert(CHUNK_GATED_DELTA_RULE_FWD_O_L1_STAGE4_END <= ArchTag::L1_SIZE,
                  "Stage4 A-prime/V resident slots exceed architecture limit.");

    __aicore__ inline ChunkGatedDeltaRuleFwdOA5CubeProcess(GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR h, GM_ADDR g,
    ...
    __aicore__ inline void Init(const ChunkGatedDeltaRuleFwdOTilingData &tiling)
    {
        tiling_ = tiling;
        ...
        for (uint32_t groupTaskIdx = coreIdx; groupTaskIdx < groupTaskNum; groupTaskIdx += coreNum) {
            ...
            if ASCEND_IS_AIC {
                ...
                for (uint32_t loadHead = 0U; loadHead < stage4TaskCount; ++loadHead) {
                    ...
                    const int64_t vOffset = ChunkGatedDeltaRuleFwdOVOOffset(tiling_, loc, hvBase + loadHead);
                    ...
                    LocalTensor<Element> l1V = resource_.l1Buf.template GetBufferByByte<Element>(
                        ChunkGatedDeltaRuleFwdOL1VOffset(loadHead));
...
private:
    ...
    __aicore__ inline void ProcessStage4Head(const ChunkGatedDeltaRuleFwdOChunkLoc &loc,
                                             ...
                                             uint32_t headOffset)
    {
        ...
        LocalTensor<Element> l1APrime =
            resource_.l1Buf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL1APrimeOffset(headOffset));
        LocalTensor<Element> l1V =
            resource_.l1Buf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL1VOffset(headOffset));
        ...
        LocalTensor<Element> l0A =
            resource_.l0ABuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0AOffset(avASlot));
        LocalTensor<Element> l0B =
            resource_.l0BBuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0BOffset(avBSlot));
        LocalTensor<float> l0C = resource_.l0CBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOL0COffset(avCSlot));
        ...
        auto tensorOlUb = tla::MakeTensor(
            resource_.ubBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOOlOffset(localSlot)), layoutOlUb,
    ...
    __aicore__ inline void ProcessStage2Head(const ChunkGatedDeltaRuleFwdOChunkLoc &loc, int64_t hk, int64_t hv,
                                             uint32_t ownerSubBlock, uint32_t localSlot,
                                             ...
                                             bool waitPreviousStage5)
    {
        ...
        if (loadQK) {
            ...
            const int64_t qOffset = ChunkGatedDeltaRuleFwdOQKOffset(tiling_, loc, hk);
            const int64_t kOffset = ChunkGatedDeltaRuleFwdOQKOffset(tiling_, loc, hk);
            ...
            LocalTensor<Element> l1Q =
                resource_.l1Buf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL1QOffset(qkL1Slot));
            LocalTensor<Element> l1K =
                resource_.l1Buf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL1KOffset(qkL1Slot));
        ...
        const int64_t hOffset = ChunkGatedDeltaRuleFwdOHOffset(tiling_, loc, hv);
        ...
        LocalTensor<Element> l1H =
            resource_.l1Buf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL1HOffset(hL1Slot));
        ...
        LocalTensor<Element> qktL0A =
            resource_.l0ABuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0AOffset(qktASlot));
        LocalTensor<Element> qktL0B =
            resource_.l0BBuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0BOffset(qktBSlot));
        LocalTensor<float> qktL0C =
            resource_.l0CBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOL0COffset(qktCSlot));
        ...
        auto tensorARawUb = tla::MakeTensor(
            resource_.ubBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOARawOffset(localSlot)), layoutARawUb,
            Catlass::Arch::PositionUB{});
        ...
        LocalTensor<Element> qhL0A =
            resource_.l0ABuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0AOffset(qhASlot));
        LocalTensor<Element> qhL0B =
            resource_.l0BBuf.template GetBufferByByte<Element>(ChunkGatedDeltaRuleFwdOL0BOffset(qhBSlot));
        LocalTensor<float> qhL0C =
            resource_.l0CBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOL0COffset(qhCSlot));
        ...
        auto tensorOSRawUb = tla::MakeTensor(
            resource_.ubBuf.template GetBufferByByte<float>(ChunkGatedDeltaRuleFwdOOSRawOffset(localSlot)), layoutOSRawUb,
            Catlass::Arch::PositionUB{});
...
#endif // CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_CUBE_H
