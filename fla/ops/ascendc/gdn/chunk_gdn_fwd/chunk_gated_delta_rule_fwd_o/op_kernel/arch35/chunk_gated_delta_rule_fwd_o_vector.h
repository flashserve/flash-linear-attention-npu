...
#ifndef CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_VECTOR_H
#define CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_VECTOR_H
...
#include "chunk_gated_delta_rule_fwd_o_common.h"
...
constexpr float CHUNK_GATED_DELTA_RULE_FWD_O_LN2 = 0.69314718055994530941723212145818f;
constexpr CastTrait CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK = {
    RegLayout::ZERO,
    SatMode::NO_SAT,
...
{
    (void)chunkLen;
    constexpr uint16_t kBt = static_cast<uint16_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
    ...
    for (uint16_t row = 0; row < kBt; ++row) {
        ...
        if constexpr (UseExp2) {
            Muls(rowGateZeroReg, rowGateZeroReg, CHUNK_GATED_DELTA_RULE_FWD_O_LN2, maskFull32);
            Muls(rowGateOneReg, rowGateOneReg, CHUNK_GATED_DELTA_RULE_FWD_O_LN2, maskFull32);
        }
...
{
    constexpr uint16_t kBt = static_cast<uint16_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
    const uint16_t activeRows = FullChunk ? kBt : validRows;
    RegTensor<float> zeroReg;
    ...
    // allowing the compiler to reuse the same physical register bank.
    {
        ...
        uint16_t rowLoop = 0;
        const uint16_t rowLoopCount = activeRows / 4U;
        const uint16_t tailLoopCount = activeRows % 4U;
        ...
        for (rowLoop = 0; rowLoop < rowLoopCount; ++rowLoop) {
            ...
            Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(aPrimeBf16Reg0, aPrimeReg0, floatMask);
            Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(aPrimeBf16Reg1, aPrimeReg1, floatMask);
            Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(aPrimeBf16Reg2, aPrimeReg2, floatMask);
            Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(aPrimeBf16Reg3, aPrimeReg3, floatMask);
        ...
        Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(aPrimeBf16Reg0, zeroReg, floatMask);
        const uint16_t invalidRowCount = static_cast<uint16_t>(kBt - activeRows);
        for (rowLoop = 0; rowLoop < invalidRowCount; ++rowLoop) {
            rowBase = static_cast<uint16_t>(activeRows + rowLoop);
            rowOffset0 = static_cast<uint32_t>(rowBase) * kBt;
            StoreAlign<bfloat16_t, StoreDist::DIST_PACK_B32>(aPrimeAddr + rowOffset0, aPrimeBf16Reg0, floatMask);
        }
    }

    // Phase 2 keeps one row and its two fixed V tiles in nested hardware loops.
    RegTensor<float> oSReg;
    ...
    constexpr uint16_t kTilesPerRow = static_cast<uint16_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_V / CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
    for (uint16_t row = 0; row < activeRows; ++row) {
        Duplicate(rowIndexReg0, static_cast<uint32_t>(row), floatMask);
        Gather(gateORowReg0, gateOVectorReg, rowIndexReg0);
        for (uint16_t tile = 0; tile < kTilesPerRow; ++tile) {
            const uint32_t offset = static_cast<uint32_t>(row) * CHUNK_GATED_DELTA_RULE_FWD_O_A5_V +
                                    static_cast<uint32_t>(tile) * kBt;
            LoadAlign(oSReg, oSAddr + offset);
            Mul(oSReg, oSReg, gateORowReg0, floatMask);
            StoreAlign(oSAddr + offset, oSReg, floatMask);
        }
    }

    for (uint16_t row = activeRows; row < kBt; ++row) {
        const uint32_t offset0 = static_cast<uint32_t>(row) * CHUNK_GATED_DELTA_RULE_FWD_O_A5_V;
...
{
    constexpr uint16_t kV = static_cast<uint16_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_V);
    constexpr uint16_t kTilesPerRow = static_cast<uint16_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_V / CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
    ...
    for (uint16_t row = 0; row < validRows; ++row) {
        for (uint16_t tile = 0; tile < kTilesPerRow; ++tile) {
            const uint32_t offset = static_cast<uint32_t>(row) * kV +
                                    static_cast<uint32_t>(tile) * CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT;
            ...
            Cast<bfloat16_t, float, CHUNK_GATED_DELTA_RULE_FWD_O_FP32_TO_B16_PACK>(oOutBf16Reg, oOutReg, floatMask);
...
class ChunkGatedDeltaRuleFwdOA5VectorProcess {
public:
    using ArchTag = Catlass::Arch::Ascend950;

    static constexpr uint32_t BANK_COUNT_2 = CHUNK_GATED_DELTA_RULE_FWD_O_STREAM_BANK_COUNT;

    __aicore__ inline ChunkGatedDeltaRuleFwdOA5VectorProcess(GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR h, GM_ADDR g,
    ...
    __aicore__ inline void Init(const ChunkGatedDeltaRuleFwdOTilingData &tiling, TPipe *pipe)
    {
        tiling_ = tiling;
        pipe_ = pipe;
        oGm_.SetGlobalBuffer((__gm__ bfloat16_t *)o_);

        pipe_->InitBuffer(ubBuf_, CHUNK_GATED_DELTA_RULE_FWD_O_UB_TOTAL_BYTES);
    ...
    __aicore__ inline void Process(uint32_t coreIdx, uint32_t coreNum)
    {
        ...
        constexpr uint32_t bt = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
        constexpr uint32_t vDim = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_V);
        const uint32_t matrixElems = bt * bt;
        ChunkGatedDeltaRuleFwdOChunkLoc loc;

        const uint32_t headGroupNum = ChunkGatedDeltaRuleFwdOHeadGroupNum(tiling_);
        ...
        for (uint32_t groupTaskIdx = coreIdx; groupTaskIdx < groupTaskNum; groupTaskIdx += coreNum) {
            ...
            ChunkGatedDeltaRuleFwdOResolveChunkLoc(cuSeqlens_, chunkOffsets_, tiling_, loopIdx, loc);
            ...
            for (int64_t headOffset = 0; headOffset < taskCount; ++headOffset) {
                ...
                if (ownerSubBlock == subBlockIdx) {
                    LocalTensor<float> gateO =
                        ubBuf_.GetWithOffset<float>(bt, ChunkGatedDeltaRuleFwdOGateOOffset(streamSlot_));
                    LocalTensor<float> gateA =
                        ubBuf_.GetWithOffset<float>(matrixElems, ChunkGatedDeltaRuleFwdOGateAOffset(streamSlot_));
                    LocalTensor<float> aRaw =
                        ubBuf_.GetWithOffset<float>(matrixElems, ChunkGatedDeltaRuleFwdOARawOffset(streamSlot_));
                    LocalTensor<float> oSPrime =
                        ubBuf_.GetWithOffset<float>(bt * vDim, ChunkGatedDeltaRuleFwdOOsPrimeOffset(streamSlot_));
                    LocalTensor<bfloat16_t> aPrimeBf16 =
                        ubBuf_.GetWithOffset<bfloat16_t>(matrixElems,
                                                         ChunkGatedDeltaRuleFwdOAPrimeBf16Offset(streamSlot_));
                    ...
                    if (loc.chunkLen == bt) {
                        ...
                        AscendC::VF_CALL<Stage3Gate64VF<false>>(
                            reinterpret_cast<__ubuf__ bfloat16_t *>(aPrimeBf16.GetPhyAddr()),
                            reinterpret_cast<__ubuf__ float *>(oSPrime.GetPhyAddr()),
                            reinterpret_cast<__ubuf__ float *>(aRaw.GetPhyAddr()),
                            reinterpret_cast<__ubuf__ float *>(gateA.GetPhyAddr()),
                    ...
                    LocalTensor<bfloat16_t> l1APrime =
                        resource_.l1Buf.template GetBufferByByte<bfloat16_t>(
                            ChunkGatedDeltaRuleFwdOL1APrimeOffset(static_cast<uint32_t>(headOffset)));
            ...
            for (int64_t headOffset = 0; headOffset < taskCount; ++headOffset) {
                ...
                LocalTensor<float> oL =
                    ubBuf_.GetWithOffset<float>(bt * vDim, ChunkGatedDeltaRuleFwdOOlOffset(streamSlot_));
                LocalTensor<bfloat16_t> oOut =
                    ubBuf_.GetWithOffset<bfloat16_t>(bt * vDim, ChunkGatedDeltaRuleFwdOOlOffset(streamSlot_));
                WaitFlag<HardEvent::MTE3_V>(mte3ToVStream_[streamSlot_]);
                PipeBarrier<PIPE_V>();
                ...
                const int64_t oOffset = ChunkGatedDeltaRuleFwdOOOffset(tiling_, loc, hv);
    ...
    __aicore__ inline void LoadStage1G(const ChunkGatedDeltaRuleFwdOChunkLoc &loc, int64_t hv, uint32_t streamSlot)
    {
        GlobalTensor<GT> gGm;
        gGm.SetGlobalBuffer((__gm__ GT *)g_);
        gGm.SetL2CacheHint(CacheMode::CACHE_MODE_DISABLE);
        const int64_t gOffset = ChunkGatedDeltaRuleFwdOGOffset(tiling_, loc, hv);
        const uint32_t bt = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
        const uint32_t gScratchOff = ChunkGatedDeltaRuleFwdOGScratchOffset(streamSlot);
    ...
    __aicore__ inline void ComputeStage1Gate(const ChunkGatedDeltaRuleFwdOChunkLoc &loc, uint32_t localSlot, uint32_t streamSlot)
    {
        const uint32_t bt = static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT);
        const uint32_t gScratchOff = ChunkGatedDeltaRuleFwdOGScratchOffset(streamSlot);
        ...
        if (loc.chunkLen < static_cast<uint32_t>(CHUNK_GATED_DELTA_RULE_FWD_O_A5_BT)) {
        ...
        LocalTensor<float> gateO = ubBuf_.GetWithOffset<float>(bt, ChunkGatedDeltaRuleFwdOGateOOffset(localSlot));
        LocalTensor<float> gateA =
            ubBuf_.GetWithOffset<float>(bt * bt, ChunkGatedDeltaRuleFwdOGateAOffset(localSlot));
        if constexpr (UseExp2) {
            Muls(gateO, gFp32, CHUNK_GATED_DELTA_RULE_FWD_O_LN2, bt);
            PipeBarrier<PIPE_V>();
            Exp(gateO, gateO, bt);
        } else {
            Exp(gateO, gFp32, bt);
...
private:
    ...
    Catlass::Arch::CrossCoreFlag vecToCubeFlag_{CHUNK_GATED_DELTA_RULE_FWD_O_VEC_TO_CUBE_READY_FLAG};
    Catlass::Arch::CrossCoreFlag cubeToVecFlag_{CHUNK_GATED_DELTA_RULE_FWD_O_CUBE_TO_VEC_READY_FLAG};
};

} // namespace GDN

#endif // CHUNK_GATED_DELTA_RULE_FWD_O_ARCH35_VECTOR_H
