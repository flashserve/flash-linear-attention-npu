...
实现类型：`ascendc`。Dtype：q/k/d_o 为 FP16/BF16，g 可为 FP16/BF16/FP32。Layout：BNSD；varlen 在 T 维拼接。模式：fixed 与 varlen；varlen 的两个索引必须同时提供。
...
fixed 以 `B*ceil(T/C)` 为 chunk 列表，varlen 直接消费规范化 `chunk_indices`；AIC 以 Q/K head 生成共享 score，AIV/AIC 以 value head 消费。
...
使用模板化 tiling：`strategy`(fixed/varlen)、`D_T_Q`、`D_T_G` 和 `V` 是有限编译期维度。key 只由模板实例生成，不承载 B/H/T 等普通 runtime shape；组合上限由 2 种策略、受支持 dtype 和 2 个 V 实例共同限定。
