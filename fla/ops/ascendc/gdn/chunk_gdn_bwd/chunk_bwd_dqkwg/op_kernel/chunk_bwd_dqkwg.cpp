...
__global__ __aicore__ void chunk_bwd_dqkwg(GM_ADDR q,             // [B, HK, T, K]
                                           GM_ADDR k,             // [B, HK, T, K]
                                           GM_ADDR v,             // [B, HV, T, V]
                                           GM_ADDR g,             // [B, HV, T]
...
)
{
    ...
    if (TILING_KEY_IS(1)) {
        ...
        if ASCEND_IS_AIV {
            ...
            ChunkBwdDqkwgVectorProcess<DTYPE_Q, DTYPE_G> vectorProcess(q, k, v, g, h, do_, dh, dv, cu_seqlens,
                                                                       chunk_indices, dq, dk, dw, dg, userWorkspace);
