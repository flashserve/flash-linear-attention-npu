# ChunkBwdDqkwg API 与调用示例

## 1. API 总览

| 通路 | API/入口 | 支持情况 |
| --- | --- | --- |
| Python 主入口 | `fla_npu.ops.ascendc.chunk_bwd_dqkwg` | 支持 |
| aclnn | `aclnnChunkBwdDqkwgGetWorkspaceSize` / `aclnnChunkBwdDqkwg` | 支持 |
| Ascend C `<<<>>>` | `chunk_bwd_dqkwg<<<blockDim, nullptr, stream>>>` | 支持 |
| legacy | `npu_chunk_bwd_dqkwg` | 支持（显式加载） |
...
aclnnStatus status = aclnnChunkBwdDqkwgGetWorkspaceSize(
    q, k, v, g, h, dox, dh, dv, nullptr, nullptr, nullptr, nullptr, scale, chunkSize, false, false, dq, dk, dw, dg, &workspaceSize, &executor);
ACLNN_CHECK(status);
void *workspace = nullptr;
if (workspaceSize != 0) {
    ACL_CHECK(aclrtMalloc(&workspace, workspaceSize, ACL_MEM_MALLOC_HUGE_FIRST));
}
ACLNN_CHECK(aclnnChunkBwdDqkwg(workspace, workspaceSize, executor, stream));
...
`tests/operators/chunk_bwd_dqkwg/routes/test_aclnn_chunk_bwd_dqkwg.cpp` 固化两段式函数签名；tensor descriptor 必须严格按本页公共
...
```python
chunk_bwd_dqkwg(q, k, v, g, h, dox, dh, dv, chunk_size, *, cu_seqlens=None, chunk_indices=None, w=None, g_gamma=None, scale=None, use_exp2=None, transpose_state_layout=None)
```
...
可编译的参数声明和 launch 包装位于 `tests/operators/chunk_bwd_dqkwg/routes/test_direct_chunk_bwd_dqkwg.cpp`。
...
负向 case 的 `expect.return_code` 与消息片段集中定义在 `tests/op_cases/chunk_bwd_dqkwg.json`，修改拦截时必须同步更新。
