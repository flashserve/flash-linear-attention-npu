...
# )
@triton.autotune(
    configs=[triton.Config({}, num_warps=16, num_stages=2)],
    key=["H", "D"],
    **autotune_cache_kwargs,
)
@triton.jit(do_not_specialize=['T'])
...
def fused_kda_gate(
    ...
    Returns:
        Output tensor of shape `[..., H, K]`.
    """
    if g.shape[-1] != head_k_dim and head_k_dim > 0:
        g = g.reshape(g.shape[:-1] + (-1, head_k_dim))
    if dt_bias is None:
        dt_bias = g_bias
...
# )
@triton.autotune(
    configs=[triton.Config({'BS': 32}, num_warps=2)],
    key=['H', 'S', 'BT', 'IS_VARLEN', 'REVERSE'],
    **autotune_cache_kwargs,
)
@triton.jit(do_not_specialize=['T'])
