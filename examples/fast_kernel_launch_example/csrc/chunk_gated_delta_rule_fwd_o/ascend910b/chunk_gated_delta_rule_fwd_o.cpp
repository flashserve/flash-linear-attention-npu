...
/*!
 * \file chunk_gated_delta_rule_fwd_o.cpp
...
#include "fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_gated_delta_rule_fwd_o/op_host/chunk_gated_delta_rule_fwd_o_tiling_processor.h"
#include "fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_gated_delta_rule_fwd_o/op_kernel/chunk_gated_delta_rule_fwd_o.cpp"

namespace ascend_ops {
namespace ChunkGatedDeltaRuleFwdO {

using TilingData = GDN::ChunkGatedDeltaRuleFwdOTilingData;

TORCH_LIBRARY_FRAGMENT(EXTENSION_MODULE_NAME, m)
{
    m.def("chunk_gated_delta_rule_fwd_o(Tensor q, Tensor k, Tensor v, Tensor h, Tensor g, float scale, int chunk_size, *, int[]? cu_seqlens=None, int[]? chunk_offsets=None) -> Tensor");
}

at::Tensor chunk_gated_delta_rule_fwd_o_meta(const at::Tensor &q, const at::Tensor &k, const at::Tensor &v, const at::Tensor &h,
...
{
    m.impl("chunk_gated_delta_rule_fwd_o", chunk_gated_delta_rule_fwd_o_meta);
}

struct ChunkGatedDeltaRuleFwdOTilingResult {
    TilingData tiling;
    uint32_t blockDim;
    size_t workspaceSize;
};

int64_t ToChunkGatedDeltaRuleFwdODtype(at::ScalarType dtype)
{
    if (dtype == at::kBFloat16) {
        return optiling::CHUNK_GATED_DELTA_RULE_FWD_O_DTYPE_BF16;
    }
    if (dtype == at::kHalf) {
        return optiling::CHUNK_GATED_DELTA_RULE_FWD_O_DTYPE_FP16;
    }
    if (dtype == at::kFloat) {
        return optiling::CHUNK_GATED_DELTA_RULE_FWD_O_DTYPE_FP32;
    }
    TORCH_CHECK(false, "Unsupported dtype: ", dtype);
    return optiling::CHUNK_GATED_DELTA_RULE_FWD_O_DTYPE_FP16;
}

ChunkGatedDeltaRuleFwdOTilingResult CalcTilingParams(const at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
                                       ...
                                       at::OptionalIntArrayRef chunk_offsets)
{
    ...
    optiling::ChunkGatedDeltaRuleFwdOTilingContext ctx{
        ...
        scale,
        chunk_size,
        ToChunkGatedDeltaRuleFwdODtype(q.scalar_type()),
        ToChunkGatedDeltaRuleFwdODtype(g.scalar_type()),
        false,
        false,
        "BNSD",
        coreNum,
    ...
    TilingData tiling{};
    optiling::ChunkGatedDeltaRuleFwdOTilingProcessor processor(ctx, tiling);
    TORCH_CHECK(processor.Process() == ge::GRAPH_SUCCESS, "chunk_gated_delta_rule_fwd_o tiling failed.");
    return ChunkGatedDeltaRuleFwdOTilingResult{tiling, coreNum, processor.GetWorkspaceSize()};
}

template <typename InputT, typename GT>
__global__ __aicore__ void chunk_gated_delta_rule_fwd_o_kernel(GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR h, GM_ADDR g,
                                               GM_ADDR cu_seqlens, GM_ADDR chunk_offsets, GM_ADDR o,
                                               GM_ADDR workspace, const GDN::ChunkGatedDeltaRuleFwdOTilingData tilingData)
{
    ...
    GDN::ChunkGatedDeltaRuleFwdOKernelImpl<InputT, GT, float>(q, k, v, h, g, cu_seqlens, chunk_offsets, o, user, &tilingData);
}

template <typename InputT, typename GT>
void LaunchChunkGatedDeltaRuleFwdO(uint32_t blockDim, aclrtStream stream, GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR h, GM_ADDR g,
                     GM_ADDR cu_seqlens, GM_ADDR chunk_offsets, GM_ADDR o, GM_ADDR workspace,
                     const GDN::ChunkGatedDeltaRuleFwdOTilingData &tiling)
{
    chunk_gated_delta_rule_fwd_o_kernel<InputT, GT><<<blockDim, nullptr, stream>>>(
        q, k, v, h, g, cu_seqlens, chunk_offsets, o, workspace, tiling);
}

at::Tensor chunk_gated_delta_rule_fwd_o_npu(const at::Tensor &q, const at::Tensor &k, const at::Tensor &v, const at::Tensor &h,
                           const at::Tensor &g, double scale, int64_t chunk_size,
                           at::OptionalIntArrayRef cu_seqlens, at::OptionalIntArrayRef chunk_offsets)
{
    const c10::OptionalDeviceGuard guard(q.device());
    auto output = chunk_gated_delta_rule_fwd_o_meta(q, k, v, h, g, scale, chunk_size, cu_seqlens, chunk_offsets);
    auto stream = c10_npu::getCurrentNPUStream().stream(false);

    ChunkGatedDeltaRuleFwdOTilingResult tilingResult =
    ...
    auto aclCall = [=]() -> int {
        if (qDtype == at::kBFloat16 && gDtype == at::kBFloat16) {
            LaunchChunkGatedDeltaRuleFwdO<bfloat16_t, bfloat16_t>(blockDim, stream, qPtr, kPtr, vPtr, hPtr, gPtr, cuSeqlensPtr,
                                                    chunkOffsetsPtr, oPtr, workspaceGm, tiling);
        } else if (qDtype == at::kHalf && gDtype == at::kHalf) {
            LaunchChunkGatedDeltaRuleFwdO<half, half>(blockDim, stream, qPtr, kPtr, vPtr, hPtr, gPtr, cuSeqlensPtr,
                                        chunkOffsetsPtr, oPtr, workspaceGm, tiling);
        } else if (qDtype == at::kBFloat16 && gDtype == at::kFloat) {
            LaunchChunkGatedDeltaRuleFwdO<bfloat16_t, float>(blockDim, stream, qPtr, kPtr, vPtr, hPtr, gPtr, cuSeqlensPtr,
                                               chunkOffsetsPtr, oPtr, workspaceGm, tiling);
        } else if (qDtype == at::kHalf && gDtype == at::kFloat) {
            LaunchChunkGatedDeltaRuleFwdO<half, float>(blockDim, stream, qPtr, kPtr, vPtr, hPtr, gPtr, cuSeqlensPtr,
                                         chunkOffsetsPtr, oPtr, workspaceGm, tiling);
        } else {
    ...
    at_npu::native::OpCommand::RunOpApi("ChunkGatedDeltaRuleFwdO", aclCall);
...
{
    m.impl("chunk_gated_delta_rule_fwd_o", chunk_gated_delta_rule_fwd_o_npu);
}
