...
#include "fla/ops/ascendc/gdn/chunk_gdn_fwd/recompute_w_u_fwd/op_host/op_tiling/recompute_w_u_fwd_tiling_processor.h"
#include "fla/ops/ascendc/gdn/chunk_gdn_fwd/recompute_w_u_fwd/op_kernel/recompute_w_u_fwd.cpp"
...
{
    m.def("recompute_w_u_fwd(Tensor k, Tensor v, Tensor beta, Tensor A, int chunk_size, *, Tensor? g=None, Tensor? gk=None, int[]? cu_seqlens=None, int[]? chunk_indices=None) -> (Tensor, Tensor)");
}

std::tuple<at::Tensor, at::Tensor> recompute_w_u_fwd_meta(
...
{
    m.impl("recompute_w_u_fwd", recompute_w_u_fwd_meta);
}
    ...
    int64_t chunk_size, at::OptionalIntArrayRef cu_seqlens, at::OptionalIntArrayRef chunk_indices)
{
    ...
    TORCH_CHECK(processor.Process() == ge::GRAPH_SUCCESS, "recompute_w_u_fwd tiling failed.");
...
__global__ __aicore__ void recompute_w_u_fwd_kernel(GM_ADDR k, GM_ADDR v, GM_ADDR beta, GM_ADDR A, GM_ADDR g,
                                                   ...
                                                   GM_ADDR workspace, const GDN::RecomputeWUFwdTilingData tilingData)
{
    ...
    if (tilingData.V == optiling::RECOMPUTE_W_U_FWD_V_DIM_256) {
...
{
    recompute_w_u_fwd_kernel<KType, BetaType, VDim><<<blockDim, nullptr, stream>>>(
...
std::tuple<at::Tensor, at::Tensor> recompute_w_u_fwd_npu(
    const at::Tensor &k, const at::Tensor &v, const at::Tensor &beta, const at::Tensor &A, int64_t chunk_size,
    const c10::optional<at::Tensor> &g, const c10::optional<at::Tensor> &gk, at::OptionalIntArrayRef cu_seqlens,
    at::OptionalIntArrayRef chunk_indices)
{
    TORCH_CHECK(g.has_value(), "recompute_w_u_fwd requires g to be provided.");
    ...
    auto outputs = recompute_w_u_fwd_meta(k, v, beta, A, chunk_size, g, gk, cu_seqlens, chunk_indices);
...
{
    m.impl("recompute_w_u_fwd", recompute_w_u_fwd_npu);
}
