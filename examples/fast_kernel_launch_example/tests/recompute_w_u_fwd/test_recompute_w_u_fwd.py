...
def run_recompute_w_u_fwd(k, v, beta, A, g, chunk_size, cu_seqlens=None, chunk_indices=None):
    return torch.ops.ascend_ops.recompute_w_u_fwd(
        k.npu(),
        v.npu(),
        beta.npu(),
        A.npu(),
...
def test_recompute_w_u_fwd_interface_exist():
    assert hasattr(torch.ops.ascend_ops, "recompute_w_u_fwd")
...
def test_recompute_w_u_fwd_fix(B, Hk, Hv, T, K, V, chunk_size, ktype, btype):
    torch.manual_seed(0)
    ...
    w, u = run_recompute_w_u_fwd(k, v, beta, A, g, chunk_size)
...
def test_recompute_w_u_fwd_variable(B, Hk, Hv, T, K, V, chunk_size, cu_seqlens_len, ktype, btype):
    torch.manual_seed(0)
    k = torch.randn(B, Hk, T, K, dtype=ktype)
    v = torch.randn(B, Hv, T, V, dtype=ktype)
    beta = torch.randn(B, Hv, T, dtype=btype)
    ...
    w, u = run_recompute_w_u_fwd(k, v, beta, A, g, chunk_size, cu_seqlens=cu_seqlens, chunk_indices=chunk_indices)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_recompute_w_u_fwd_variable_metadata_pair_required():
    torch.manual_seed(0)
    ...
    with pytest.raises(RuntimeError, match="cu_seqlens and chunk_indices"):
        run_recompute_w_u_fwd(k, v, beta, A, g, chunk_size, cu_seqlens=cu_seqlens, chunk_indices=None)

    with pytest.raises(RuntimeError, match="cu_seqlens and chunk_indices"):
        run_recompute_w_u_fwd(k, v, beta, A, g, chunk_size, cu_seqlens=None, chunk_indices=[0, 0])
