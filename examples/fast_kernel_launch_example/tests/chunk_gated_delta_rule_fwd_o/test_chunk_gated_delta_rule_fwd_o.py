...
def chunk_gated_delta_rule_fwd_o_ref(q, k, v, h, g, scale, chunk_size, cu_seqlens=None, chunk_offsets=None):
    out_dtype = v.dtype
...
def test_chunk_gated_delta_rule_fwd_o_interface_exist():
    print(torch.ops.ascend_ops.chunk_gated_delta_rule_fwd_o)
    assert hasattr(torch.ops.ascend_ops, "chunk_gated_delta_rule_fwd_o")
...
def test_chunk_gated_delta_rule_fwd_o_fix(B, HK, HV, T, K, V, chunk_size, scale, input_dtype, g_dtype):
    q, k, v, h, g, cu_seqlens, chunk_offsets = make_inputs(
        B, HK, HV, T, K, V, chunk_size, input_dtype, g_dtype, variable=False
    )
    expected = chunk_gated_delta_rule_fwd_o_ref(q, k, v, h, g, scale, chunk_size, cu_seqlens, chunk_offsets)

    actual = torch.ops.ascend_ops.chunk_gated_delta_rule_fwd_o(
...
def test_chunk_gated_delta_rule_fwd_o_variable(B, HK, HV, T, K, V, chunk_size, scale, token_batch, input_dtype, g_dtype):
    q, k, v, h, g, cu_seqlens, chunk_offsets = make_inputs(
