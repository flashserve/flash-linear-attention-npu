...
- `tests/atk/README.md`：Ascend C 单算子 ATK 自动化、用例资产和 TilingKey 覆盖要求。
- `.github/pull_request_template.md`：PR 必填信息和验证矩阵。
- 当前修改算子的 `README.md`、`docs/aclnn*.md`、`tests/atk/<op>/README.md`、测试脚本和相邻算子实现。
...
本规则统一适用于仓库内所有算子。算子原型、aclnn 接口和 `fla_npu.ops.ascendc.<op_name>` 接口一旦确定，任何功能开发、问题修复、精度或性能优化、tiling、平台适配和重构都不得修改以下内容：
...
任何确需演进上述原型或接口的方案，都必须在 PR 描述中说明：修改前后的完整接口、为什么一定要修改、为什么不能通过已有输入或属性、InferShape、origin/view/storage shape、tiling data、workspace、内部结构或内部算子获得所需信息，以及 ABI、源码和用户调用兼容性影响。不得以“内部属性”“带默认值”“只影响 L0”或“调用方可以适配”为由规避兼容性说明和对应评审。

对于已经发布的原型或接口，默认优先保留原接口并新增独立 V2 实现，由旧接口继续维持原始默认行为；V2 的必要性、完整原型和迁移策略必须在 PR 描述中说明，不得直接在原接口上做不兼容演进。
...
4. **实现信息内聚**：新增的 shape、平台能力、模板选择、tile、任务划分和 workspace 等实现信息应由 host tiling 推导，并通过 tiling data、workspace 或编译期模板参数传给 kernel，不得编码成冗余 L0 入参。
5. **既有输入保持**：融合只改变执行图、阶段组织和中间量生命周期，不得删除、改名、重排或改变既有输入、可选输入、属性和外部元数据的语义与信息来源。调用方已持有或原路径已显式传入的信息必须继续从外部传入；例如 `cu_seqlens`、`chunk_indices` 及其他既有索引映射不得为了减少参数而改为 host tiling 或 kernel 内二次推导。只有原接口未提供且纯属实现策略的信息才按上一条内聚到 tiling 或内部模板。
6. **设计说明**：新增或修改 L0 原型、拆分或融合 L0、增加 V2 L0，或确需保留多条 L0 路径时，必须在 PR 描述中说明修改前后的调用图和原型、复用现有 L0 不可行的原因、信息来源与传递方案、泛化与性能影响、ABI 影响，以及旧路径迁移或删除计划；默认方案是只保留一套 L0 路径。
...
- `tests/`：工程级 UT 和测试资产。
- `tests/atk/`：Ascend C 单算子 ATK 用例、executor、生成配置和统一执行入口。
...
Ascend C 单算子验证优先使用 `tests/atk/run_test_cpu.sh`；具体用例限制和参数从 `tests/atk/README.md` 与当前算子的 `tests/atk/<op>/README.md` 获取。旧 `torch_custom/fla_npu/test` 用例用于 ABI、公开入口、legacy 路径兼容或尚无 ATK 工程的算子，不能替代已有 ATK 工程的精度验证。Example/ST 和 CI 命令从 `README.md`、当前算子 README 和现有脚本选择。缺少 CANN、NPU 或运行时依赖时，不得把未执行命令写成已验证结论。

## 测试要求

- 开始实现前，以目标基线和当前分支的 diff 识别受影响算子。直接修改算子目录时映射到同名 `tests/atk/<op>`；修改公共组件、ABI、代码生成模板或跨算子路径时，必须列出并验证所有受影响算子，不得因无法自动映射而静默跳过。
- 已有 ATK 工程的算子必须把 `README.md`、`<op>.yaml`、`gen_<op>.py`、`atk_<op>.json` 和 `executor_<op>.py` 作为完整测试契约维护。新增算子应在交付前补齐这组资产。
- ATK 必须从 `https://gitcode.com/Ascend/ATK` 官方开源仓构建安装，版本不得低于 `26.8.8`；准备验收时必须记录实际 `atk --version` 和源码 commit。版本不满足时应停止测试并修复环境，不得通过跳过新参数降级运行。
- 开发过程中每次修改只强制执行“更新必要用例 -> 构建安装 -> ATK 精度”；精度通过才算一次有效开发迭代，并记录本次改进点和精度结果。准备验收该轮产物时，才对选定的精度通过版本执行性能、确定性和 mssanitizer，记录最终性能与完整专项结论；不得用精度失败版本的性能结果推进结论。
- 新增或修改 TilingKey、模板选择或 tiling 分支时，必须枚举全部可达 TilingKey，并在算子 ATK README 中维护“TilingKey、选择条件、普通 case、边界 case、目标 SOC、实际命中证据”的映射。输入 shape 只能证明预期选择，不能代替 host tiling UT 或运行时 tiling 日志中的实际命中证据。
- 修改参数校验、shape、dtype、layout、range、平台差异或预留参数语义时，补充反向测试和边界测试。
- 修改 Kernel 时至少覆盖对应 ATK 单算子测试；涉及 GDN 端到端链路时跑 Example/ST。
...
- `tests/atk` 是仓内可执行的单算子自动化，GitHub NPU CI 是独立门禁；除非 workflow 明确调用 ATK，否则不得把 CI 通过写成 ATK 已执行，也不得假定 push 会自动运行 ATK。
