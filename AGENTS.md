...
- scoped run 包只替换当前 wheel 内的 OPP/opapi/kernel/proto 等产物，不会刷新已安装 wheel 的 Python wrapper。正式四输出路径若依赖仓内 ctypes wrapper 传递 dummy debug 参数，验证命令必须显式设置 `PYTHONPATH=<repo>/torch_custom/fla_npu:${PYTHONPATH}`，同时在 Python 启动前 prepend 当前 wheel OPP 的 `op_api/lib` 到 `LD_LIBRARY_PATH`；否则旧 installed wrapper 可能仍按 legacy torch dispatcher 路径传空 debug 输出，表现为 `debugKktOut must not be nullptr` 或 `aclnnPrepareWyReprBwd ... not in libopapi.so`。
...
- GVA 场景下 `KKT` 必须在当前 2-head 窗口内按 `HK` 去重生产：窗口内遇到新 `hk` 才计算一次，debug/golden 按窗口内 `HK` 语义比对；同一窗口内相同 `hk` 的后续 `hv` 不再重复做 KKT GEMM。当前 4-slot 方案使用四块 window-scoped KKT cache，并把每个 head workspace slot 映射到对应 `kktSlotForSlot_[slot]`；跨窗口不共享 KKT cache，避免下一窗口覆盖上一窗口 final output 仍在读取的 cache。stage0 已验证 KKT 尾部需要同时闭环 L0C/fixpipe 和本次 KKT 使用的 L0A/L0B free 事件：`copyL0CToGm(KKT)` 后等待并重新设置 `FIX_M`，再等待并重新设置对应 `M_MTE1`，否则多 head 或多 chunk 下可能出现 `L0A/L0B read/write conflict`。
- 当前 `A` 已经作为独立 L1 resident 复用到 stage0 `Dkbg/Dvb` 和 stage2 `DA5/DA6_T`，`Dkbg/Dvb/KKT` 仍在 stage0 生产。后续若继续改这些矩阵的计算顺序、resident 生命周期或输出位置，需要用正式四输出 golden 或专门 debug 输出覆盖 `dk/dv/dbeta/dg` 中实际消费这些中间量的路径，不能只看早期 `DA4/D` debug 结果。
