...
- A5/`arch35` Vector UB 可用空间按 248KB 规划；在此约束下，`DK=D@Kbeta` 的 L0C->UB 试验路径允许单独分配两块 16KB DK CV UB，不占用 matrix input ping/pong，也不使用 matrix input 的 `MTE2_V/V_MTE2` HardEvent。Cube stage3 先写 GM `Dkb` 并 set 通用 `cubeToVecFlag` 让 Vector final 开始，随后将 `DK` row tile Fixpipe 到 DK CV UB；Vector final 等 DK CV ready 后直接消费 UB 中 `DK` 合成 `dk`，消费后 set DK CV free。DK CV 使用独立于 DA6 CV 的 x4 ready/free flag id，不能和通用 C->V/V->C GM ready flag 混用。
