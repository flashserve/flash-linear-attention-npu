...
- 正式四输出精度验收使用 `fla/ops/ascendc/gdn/chunk_gdn_bwd/prepare_wy_repr_bwd/test/test_final_golden.py` 的默认 CPU dual 模式：actual 来自 `fla_npu.ops.ascendc.prepare_wy_repr_bwd`，GT 使用 CPU 高精度 `prepare_wy_repr_bwd_da + prepare_wy_repr_bwd_full` 公式链，benchmark 使用 CPU normal `prepare_wy_repr_bwd_da + prepare_wy_repr_bwd_full` 公式链，比较使用 `ct.dual`，不要另写一套阈值或 bad-count 规则。这类脚本不能称作 ATK 对比；ATK 适合正式 API/全量泛化精度。
- 正式四输出 golden 的超时结论必须分清 `kernel` 超时和 CPU golden/compare 超时。测试脚本应先单独执行待测 op 并 `torch.npu.synchronize()`，打印 `kernel_sync` 耗时；如果没有打印该阶段结果，才按 kernel 未返回/疑似 kernel 超时定位。如果 `kernel_sync` 已返回但后续超时，应按 CPU high precision/normal golden 生成或逐输出 `ct.dual` 比对超时处理，不能误报为 kernel 超时。全量 run 期间如果日志超过 100 秒没有出现新的 case 行、`phase_time kernel_sync` 或结果行，可按最后一个未完成阶段判定卡死；最后日志已经进入某 case 但没有打印 `kernel_sync` 时，按该 case 的 kernel 阶段卡死处理。必要时先跑 `--kernel-only` 只确认 kernel 返回，再跑完整精度比对。
- NPU `prepare_wy_repr_bwd_da + prepare_wy_repr_bwd_full` 只作为显式兼容/排查模式保留：需要时运行 `test_final_golden.py --golden-mode npu-single`，使用 `ct.single` 对比 fused 与 NPU da+full 链路，但该结果不作为当前 fused 算子的正式精度验收标准。正式四输出入口接入后，按“典型优先、明确要求才全量”的规则验证正式输出。阶段测试默认输入为对称随机分布 `[-1, 1]`；如需缩小定位数值问题，可临时调整 `--input-scale`，但最终结论必须回到默认分布。
- 默认精度验证不跑完整 F1-F22、L1-L12；只有用户明确要求“全量验证”时，才运行 `test_final_golden.py --suite all`。日常精度修复和验收优先选择典型 case 集，必须覆盖 GVA 1:2、1:3，`chunk_size=64/128`，`V=128/256`，以及变长尾块；默认使用 `test_final_golden.py --suite smoke`，该典型集使用较小 shape 覆盖上述组合，避免正式四输出和 CPU high precision golden 在长序列大用例上默认占用过多内存和时间。若怀疑执行卡死，先用同一典型集加 `--kernel-only` 确认 kernel 返回；只有典型集失败或用户要求时再扩大到全量。最终 golden 脚本默认隐藏逐输出 `ct.dual` 明细，只打印失败摘要和进度；需要查看原始 CT 报告时再加 `--verbose-ct`。`--input-scale` 表示 `[-scale, scale]` 的半宽，默认 `1.0`。
