...
- 正式验收比较标准使用用户提供 golden 的 `ct.dual(actual.cpu(), cpu_high, cpu_normal, dtype=ktype)`，不要另写阈值、bad-count、`ct.single` 或自定义误差规则。A2/A5 交叉验证时优先跑典型 case `0,1,2,5,10,13`，覆盖 fixed/varlen、`chunk_size=64/128`、`V=128/256`、GVA 1:2/大比例和尾块；只有用户明确要求“全量验证”时才跑全部启用 case。CPU golden 可能很慢，尤其大 GVA case，不能把 CPU golden 或 `ct.dual` 长时间运行误判为 kernel 卡死。
...
- A5 上验证时必须显式设置当前 run 包对应的 `FLA_NPU_OPP_PATH`，并在 Python 内确认 `_resolve_vendor_dir()` 指向本次安装的 vendor 目录；如果误命中 CANN 包内 stale `custom_transformer`，可能出现 fused 四个输出全 0 且 `ct.dual` 全 FAIL，这不是有效精度结论，先修 OPP 路径再测。
- A5 上用户提供 golden 脚本依赖 `ct`，若环境没有默认安装，需要把可用的 `ct` Python 路径加入 `PYTHONPATH` 后再运行；缺少 `ct` 的 `ModuleNotFoundError` 属于测试环境问题，不是算子问题。
- A5 上跑精度前必须先看 `npu-smi info` 并选择状态正常、空闲的卡，设置 `ASCEND_RT_VISIBLE_DEVICES` 后用最小 `torch.npu.set_device(0)`/空 op 验证可用。已遇到过物理 0 卡处于 `Alarm`、部分卡 `set_device` 长时间不返回或 kernel timeout 的情况，这类结果不能作为融合算子精度失败结论。
