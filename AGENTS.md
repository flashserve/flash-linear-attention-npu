...
- Cube 侧同一个 L0C 计算结果只允许执行一次 Fixpipe copyout，不能从同一份 L0C 连续 Fixpipe 到多个目的地。正式路径只写 workspace 或最终输出；若临时定位需要额外中间结果，必须先说明方案并获得用户确认，验证结束后删除临时输出通路。
...
- `prepare_wy_repr_bwd` stage2 已确认采用 `DA6_T` 物理布局：数学公式仍是 `DA6=A.T@DA5`、`D=(-DA6*exp(min(g_i-g_j,0))*tril).T`，但 Cube 在 workspace `da6Offset` 写入 `DA6_T=DA6.T=DA5.T@A`，Vector 连续行读取后计算 `D[p,q]=-DA6_T[p,q]*exp(min(g[q]-g[p],0))*upper_tri(q>p)`，避免 Vector 侧转置散写。正式精度验收使用 CPU dual 标准，避免 A5 等环境中 NPU da/full 小算子链路差异影响 fused 算子结论。
