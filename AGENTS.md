...
- 新 `prepare_wy_repr_bwd` 的核间同步当前允许使用不带 reverse 的 Catlass raw flag：生产方使用 `Arch::CrossCoreSetFlag<0x2, PIPE_*>(flag)`，消费方使用 `Arch::CrossCoreWaitFlag(flag)`。该结论依赖当前 2-head window 和严格 stage 顺序：同一方向每个同步点连续未消费 set 数不超过 `headCnt <= 2`，不会触发同一 flag 连续 set 超过 15 次的风险。后续若扩大 head window、跨 window 前移 stage、改成 credit 流水、增加无消费预置 set，或让某一侧可连续跨多个 task/window 领先，必须重新评估是否恢复 `WithReverse` 或引入显式 credit/free 协议。
- 新 `prepare_wy_repr_bwd` 的 Cube/Vector 双向同步使用方向独立的 `CrossCoreFlag`：Vector->Cube 和 Cube->Vector 各一套 ready flag；四个 GM workspace slot 不再额外拆 CrossCoreFlag。当前跨核同步只保留 6 个数据 ready 点，final output 完成后不再发送额外 V->C slot free 信号。
...
- `CrossCoreFlagWithReverse` 不会在每次 `Set/Wait` 后自动切到 `reverseId`；它只是在连续 `Set` 或连续 `Wait` 达到 `REVERSE_DEPTH` 后才用 `reverseId` 做反向握手防止 flag 溢出。若某算子改用 raw `CrossCoreFlag`，必须先证明同一 flag 的连续未消费 `Set` 深度小于硬件限制，并在方案说明中列出每侧 `Set/Wait` 次数、方向、同步粒度和背压来源，询问用户确认后再改代码。
