#!/usr/bin/env python3
"""Run the canonical Ascend C accuracy backend for every registered operator."""
...
def main() -> int:
    ...
    parser.add_argument("--timeout", type=int, default=1800, help="per-operator timeout in seconds")
    ...
    for op in requested:
        ...
        print(f"[ACCURACY] soc={args.soc} op={op}", flush=True)
        print("[ACCURACY] " + " ".join(shlex.quote(item) for item in command), flush=True)
        if args.dry_run:
            continue
        ...
        try:
            ...
            try:
                ...
                process.wait()
            failures.append((op, f"timed out after {args.timeout}s"))
            continue
        if return_code != 0:
            failures.append((op, f"pytest returned {return_code}"))
        else:
            print(f"[ACCURACY][PASS] {op}", flush=True)

    if failures:
        for op, reason in failures:
            print(f"[ACCURACY][ERROR] {op}: {reason}", file=sys.stderr, flush=True)
        return 1
    action = "planned" if args.dry_run else "passed"
    print(f"[ACCURACY] {action} {len(requested)} operators on {args.soc}", flush=True)
    return 0
