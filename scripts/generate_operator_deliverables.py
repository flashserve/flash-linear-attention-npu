...
OPS = {
    "chunk_bwd_dqkwg": {
        ...
        "modes": "fixed 与 varlen；varlen 的两个索引必须同时提供",
        "limits": [
        ...
        "task": "fixed 按 `B*N_c` 分配 chunk，varlen 按 `chunk_indices` 分配；每个 chunk 内再按 value head 划分 AIC/AIV 工作。",
        ...
        "errors": rows(
            ...
            ("w/g_gamma 非空，或 varlen 元数据只提供一个", "ACLNN_ERR_PARAM_INVALID"),
    ...
    "chunk_bwd_dv_local": {
        ...
        "task": "fixed 以 `B*ceil(T/C)` 为 chunk 列表，varlen 直接消费规范化 `chunk_indices`；AIC 以 Q/K head 生成共享 score，AIV/AIC 以 value head 消费。",
        "tiling_key": "使用模板化 tiling：`strategy`(fixed/varlen)、`D_T_Q`、`D_T_G` 和 `V` 是有限编译期维度。key 只由模板实例生成，不承载 B/H/T 等普通 runtime shape；组合上限由 2 种策略、受支持 dtype 和 2 个 V 实例共同限定。",
    ...
    "chunk_gated_delta_rule_bwd_dhu": {
        ...
        "modes": "fixed/varlen、标量 g、FP16/BF16 主张量、FP16/BF16/FP32 gate",
        "limits": [
...
OPS.update({
    "chunk_fwd_o": {
        ...
        "modes": "fixed/varlen、GVA、整块/尾块；g 为必选标量 gate",
        "limits": [
        ...
        "tiling_key": "不使用 tiling key 分派 shape。主 dtype、gate dtype、fixed/varlen、K/V/C 均写入 tiling data，kernel 通过统一 Catlass 调度器处理，避免把 B/H/T 组合固化为 key。",
    ...
    "chunk_gated_delta_rule_fwd_h": {
        ...
        "modes": "fixed/varlen、g/gk、可选 initial/final state",
        "limits": [
            "`K` 仅支持 128，`V` 仅支持 128/256，`chunk_size` 仅支持 64/128。",
        ...
        "tiling_key": "仅保留 2 个 key：V<=128 选择 key 1 的 128 列 Cube tile，128<V<=256 选择 key 2 的 256 列 tile。必须使用 key 是因为两种 Cube tile 和 KERNEL_TASK_TYPE 在编译期不同；dtype、g/gk、initial/final、fixed/varlen 均由 tiling data 处理，不继续扩张 key。",
    ...
    "recompute_wu_fwd": {
        ...
        "inputs": rows(
            ...
            ("cu_seqlens", "可选", "[N+1]", "INT64", "ND", "varlen 累计长度"), ("chunk_indices", "可选", "[2*N_c]", "INT64", "ND", "展平 chunk 索引"),
        ),
        ...
        "modes": "fixed/varlen、GVA、标量 gate g",
        "limits": [
    ...
    "solve_tri": {
        ...
        "dtype": "FP16/BF16",
        "modes": "dense BHTD/BSND 与 varlen TND，支持尾块",
        "limits": [
        ...
        "tiling_key": "固定使用 key 1 作为既有 kernel launch ABI，key 不承载 shape 或模式分派。C、dtype、layout、尾块和 varlen 全部由 tiling data 处理，因此不会随 B/H/T 产生组合爆炸；保留 key 1 是因为 kernel 入口现有 TILING_KEY_IS(1) 编译契约。",
...
OPS.update({
    "causal_conv1d": {
        ...
        "title": "CausalConv1d",
        "family": "gdn",
        "purpose": "GDN 输入预处理的因果一维卷积。run_mode=0 执行 dense/varlen 前向并维护卷积状态，run_mode=1 执行 decode/投机更新；可选 SiLU 激活和 head layout 转换。",
        "math": dedent(r"""
            对通道 d 和时间 t：
        ...
        "modes": "dense/varlen forward、decode/update、投机接受 token",
        "limits": [
    ...
    "causal_conv1d_bwd": {
        ...
        "family": "gdn",
        "purpose": "CausalConv1d 的训练反向算子。它支持 dense/varlen 和五种公开 layout，根据 x/weight/dy 及可选预激活 y、initial_state、dht 计算 dx/dw/db/dh0。",
        "math": dedent(r"""
        ...
        "modes": "dense/varlen、无激活/SiLU、可选初末状态",
        "limits": [
        ...
        "task": "dx 按 token/channel tile 计算，dw/db 按 channel/width 分核并在 workspace 归约；varlen 不跨序列边界读取。",
    ...
    "chunk_local_cumsum": {
        ...
        "purpose": "在每个时间 chunk 内对 gate 或任意尾部特征做局部前缀/后缀累加。该 Ascend C 实现替换 Triton 预处理路径，并支持 fixed/varlen 与 scale。",
        "math": dedent(r"""
            将 token 后的尾部维展平为 P：
        ...
        "modes": "fixed/varlen、forward/reverse、任意连续尾部 P",
        "limits": [
        ...
        "task": "fixed 按 `(B*H_v,chunk,tail_tile)` 分配；varlen 按 `(seq_id,local_block_id,head,tail_tile)` 分配，每个处理块内再按 C 完成 scan。",
        "tiling_key": "未使用多分支 tiling key；reverse、scale、tail P、fixed/varlen 都由 tiling data 和 runtime 常量处理。",
        ...
        "local_symbols": [("N_b", "varlen 内部处理块总数；由各序列 ceil(seq_len/B_T) 求和"),
                          ("B_T", "单个 varlen 处理块覆盖的 token 数，由 tiling 根据 UB、C、P 计算")],
        "errors": rows(
    ...
    "chunk_scaled_dot_kkt": {
        ...
        "modes": "fixed/varlen、GVA 输入、整块/尾块",
        "limits": [
    ...
    "kda_gate_cumsum": {
        ...
        "modes": "dense/varlen、step-gate/safe raw-gate、四种 layout、整块/尾块",
        "limits": [
        ...
        "task": "dense 按 (B,H_v,chunk) 分配，varlen 按 (sequence,H_v) 分配并在 core 内遍历该序列真实 chunk；每个 task 的 K 行按最多 256 元素的 UB 向量处理。",
...
def varlen_note(spec):
    ...
    return (
        "varlen 模式中，`cu_seqlens[0]` 必须为 0、末项等于 `T` 且序列非递减。"
        + chunk_note
        + "fixed 与 varlen、尾块与整块遵循同一数学定义。"
    )
...
def render_design(op, spec):
    ...
    if has_input(spec, "cu_seqlens"):
        boundary = (
            "dense/fixed 尾块与 varlen 尾段均按每条逻辑序列的有效长度计算，任何补齐元素在参与指数、"
...
def enrich_existing_manifest(op, spec, manifest):
    ...
    manifest["data_generation"] = manifest.get(
        "data_generation",
        {
            "floating_inputs": {"distribution": "uniform", "min": -1.0, "max": 1.0},
            "positive_inputs": {"distribution": "uniform", "min": 0.0, "max": 1.0},
            "structured_inputs": "由 case.optional_inputs 和 reference runner 构造 gate、状态及 varlen 索引",
        },
    )
