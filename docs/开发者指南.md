...
bash build.sh --soc=ascend910b --pkg --vendor_name=fla_npu --ops=chunk_gated_delta_rule_fwd_o
...
```sh
FLA_NPU_SOC=ascend910b FLA_NPU_OPS=chunk_gated_delta_rule_fwd_o,chunk_bwd_dv_local \
...
列出 diff 的算子目录与文件（如 `ascend910b/chunk_gated_delta_rule_fwd_o/ChunkGatedDeltaRuleFwdO_*.o`）。
