...
一起报出来（`fla_npu(stable): size_of dim 1 out of range at csrc/src/stable_causal_conv1d_bwd.cpp:74 (tensor weight_meta, ndim=1, shape=[1536])`），
没传的可选入参直接点名：`(tensor g_meta is None / undefined)`。名字和行号都由宏带上，调用点不用传额外东西
（实参里带逗号时自己加一层括号）。名字只能是**宏展开那一行的实参原文**，所以新加 helper 时**不要在 helper
里读维度**（在函数里只能拿到形参名，报出来就是别的张量）：让 helper 回答"该读第几轴"，取维写回算子自己
那行——`SIZE_OF(q_meta, layout_math::token_axis(layout))`。
