...
3. **迁移代码调用**：将旧入口替换为 `fla_npu.ops.ascendc` 下对应接口，例如：
   ...
   | 旧（≤ v26.6.0）                                        | 新                                                                    |
   | ------------------------------------------------------- | --------------------------------------------------------------------- |
   | `torch.ops.npu.npu_chunk_gated_delta_rule_fwd_o(...)`                  | `from fla_npu.ops.ascendc import chunk_gated_delta_rule_fwd_o; chunk_gated_delta_rule_fwd_o(...)`     |
...
- **安装验证命令**：旧版用 `fla_npu.is_legacy_torch_ops_loaded()` 和 `hasattr(torch_npu.ops, 'chunk_gated_delta_rule_fwd_o')` 验证，新版不再适用（见根 README 的 Step 4 说明）；统一改用 `python -c "import fla_npu; print('ok')"` + `python scripts/check_packaged_wheel_api.py`。
...
- **新版 wheel**：默认不注册 `torch_npu.ops.*` 兼容入口，`torch_npu.ops` 属性本身不存在，
  旧版验证命令 `hasattr(torch_npu.ops, 'chunk_gated_delta_rule_fwd_o')` 会抛 `AttributeError` 而非返回
