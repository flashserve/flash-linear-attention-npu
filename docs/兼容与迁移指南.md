...
**v26.6.0 之后不再维护旧版本兼容接口**。`fla_npu.ops.ascendc` 默认通过 Stable-ABI 适配层
（`libfla_npu_stable.so`）调用 `libcust_opapi.so` 里的 aclnn 符号：适配层只依赖
`torch/csrc/stable` 的 C 面，不绑 CPython ABI、也不绑 libtorch C++ ABI，接口语义稳定、
后续版本不会变动。纯 Python `ctypes` 直调（`_aclnn_ctypes.py`）保留为参考实现与回退，
用 `FLA_NPU_STABLE_ABI=ctypes` 切换。`torch.ops.npu.*` / `torch_npu.ops.*` 仅作为迁移期临时兼容能力保留，不推荐
新增代码使用，也不会默认使能。
...
2. **安装新版本 wheel**：见根 [README](../README.md) 的 Step 2 / Step 3。
...
- **安装验证命令**：旧版用 `fla_npu.is_legacy_torch_ops_loaded()` 和 `hasattr(torch_npu.ops, 'chunk_fwd_o')` 验证，新版不再适用（见根 README 的 Step 3 说明）；统一改用 `python -c "import fla_npu; print('ok')"` + `python scripts/check_packaged_wheel_api.py`。
