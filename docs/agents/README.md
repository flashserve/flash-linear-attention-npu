...
- [`reference/04-operator-development/工程结构规范.md`](reference/04-operator-development/工程结构规范.md) 维护算子的工程结构规范：目录职责、`def`/`aclnn`/调用层契约、`arch22`/`arch35` 与模板参数、测试看护和编码细节；同目录的 [`标准工程示例/`](reference/04-operator-development/标准工程示例/) 给出逐文件对照与骨架。阶段 4 编码前读取，检视时作为判定依据。
...
| 任务 | 必读文件 |
| --- | --- |
| 新增算子、调整工程结构（目录、`def`、`aclnn`、调用层、模板） | [`reference/04-operator-development/工程结构规范.md`](reference/04-operator-development/工程结构规范.md) 与同目录 [`标准工程示例/`](reference/04-operator-development/标准工程示例/)；涉及接口迭代时同时读取当前算子的 `docs/api.md` |
...
- 工程结构规则只写在 `reference/04-operator-development/工程结构规范.md`；精度静态检视的分级、类别、流程与验收规则只写在 `skills/op-precision-review/`。
