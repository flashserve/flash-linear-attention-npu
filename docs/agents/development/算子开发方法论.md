...
本文面向 AI coding agent 和开发者，说明在本仓开发 AscendC 算子时应该怎样拆问题、定边界、写实现和做验证。这里优先沉淀通用方法论；具体算子的设计细节必须先写入算子目录下的设计文档，再由 README、代码和测试共同实现。

首次参与本仓开发时先读 [`../基础知识.md`](../基础知识.md)。任何算子开发、优化或检视开始前先执行 [`算子开发通用约束.md`](算子开发通用约束.md)，按 [`算子开发设计文档模板.md`](算子开发设计文档模板.md) 在对应算子目录完成 `docs/算子开发设计.md`，编写或重构 Ascend C 代码前再阅读 [`算子编码规范.md`](算子编码规范.md)。涉及性能设计、stage、resident、head window、流水或 SOC 优化时，按 [`../optimization/README.md`](../optimization/README.md) 加载对应分类文档。

## 设计先行

设计文档是实现前的工程契约，不是出问题后的复盘材料。设计阶段至少完成以下内容：

1. 固定目标、非目标、支持范围、性能基线和模板优势域，区分功能边界与性能锚点。
2. 列出完整公开接口和输入来源，建立融合入口参数到原调用链的映射；保留所有已有输入、属性和外部元数据。
3. 按数学语义列出公式、dtype/累加精度、有效区域、返回值和状态语义。
4. 画出 producer-consumer DAG，并将节点按“chunk 间无依赖”和“chunk 间有依赖”分类；后者继续标记正向/反向 carry、owner、初始状态、更新点和同步。
5. 根据依赖决定 stage、kernel 边界、AIC/AIV 分工、head window、L1/UB/workspace 位置和生命周期，给出容量核算与复用条件。
6. 静态枚举 tiling/template 分支，建立每个 key 的普通 case、边界 case、SOC 和实际命中证据。
7. 提前列出 ATK accuracy、performance、determinism、mssanitizer、ABI 和 Example/ST 的验收矩阵，以及每轮迭代的记录格式。

完成后的文档必须保存在 `fla/ops/ascendc/.../<operator>/docs/算子开发设计.md`。参考 GDN backward finalize 设计的完整章节组织方式，但必须重新推导当前算子的资源数字、依赖边界和执行策略；不能把参考设计当作现成实现方案。
...
Ascend C 算子应在开始实现前根据目标基线与当前分支的 diff 确定受影响算子，并将每个算子映射到 `tests/atk/<op>`。算子私有目录通常映射到同名 ATK 工程；公共组件、ABI、代码生成模板或跨算子路径的改动必须先列出所有调用方，再展开为多个 ATK 工程。无法可靠映射时应显式记录缺口并补充测试资产，不能静默跳过。

开发过程分为快速迭代和轮次验收两个阶段：

```text
更新 YAML、生成器和覆盖矩阵
  -> 显式执行 gen_cases，并评审生成结果
  -> 更新受版本控制的 atk_<op>.json
  -> 构建并安装当前代码
  -> ATK accuracy
  -> accuracy 通过，本次开发迭代成立
  -> 选定准备验收的精度通过产物
      -> ATK performance
      -> ATK determinism
      -> ATK mssanitizer
      -> 按风险扩大到 ABI、Example/ST 和 NPU CI
```

`gen_cases` 与日常执行分离：生成动作会产生待评审用例，不应在每次 `all` 时覆盖已评审 JSON。快速开发期间每次只需执行 `accuracy`；当前代码、当前用例和当前构建产物通过精度后，记录本次改进点和精度结果。准备验收该轮产物时，再对选定版本执行完整专项测试并记录最终性能。精度失败的尝试可以记录根因和回退，但不计为完成迭代。详细命令和结果要求见 [`验证方法与矩阵.md`](验证方法与矩阵.md)。
...
本规则统一适用于仓库内所有算子。算子原型、aclnn 接口和 `fla_npu.ops.ascendc.<op_name>` 接口一旦确定，必须作为兼容性契约保持稳定。任何开发或修复都不得直接修改以下内容：
...
确需演进接口时，必须在 PR 描述中说明拟修改前后的完整接口、为什么一定要修改，以及为什么不能通过已有输入或属性、InferShape、origin/view/storage shape、tiling data、workspace、内部结构或内部算子获取所需信息。还必须说明 ABI、源码和用户调用兼容性影响，并由代码评审确认迁移方案；不能先提交不兼容实现，再补兼容性说明。

对于已经发布的原型或接口，默认优先保持原接口不变并新增独立 V2 实现，让旧接口继续维持原始默认行为。新增 V2 时必须在 PR 描述中给出必要性、完整原型、迁移计划和旧接口维护策略，并由代码评审确认；不得直接在原接口上做不兼容演进。
...
1. L0 输入、输出和属性表达算子语义以及现有调用链已经显式传入的信息；融合和优化不得借“可以重新推导”删除、改名、重排既有参数或改变其信息来源。
2. 对原接口未提供的新实现信息，可从 tensor descriptor、已有属性或平台能力获得的 shape、layout、dtype、SOC 和模板选择信息，由 InferShape 或 host tiling 推导。
...
5. 调用方已经持有或原路径已经显式传入的元数据必须继续从外部传入。`cu_seqlens`、`chunk_indices` 及其他既有索引映射只允许在 host tiling 中校验、解析和转发，不得为减少参数而重新生成。

### 设计说明规则

新增 L0 原型、修改已有 L0 签名、增加 V2 L0、拆分或融合 L0，或保留多条 L0 调用路径，都必须在 PR 描述中给出可评审的设计说明。设计说明至少包含：
...
大融合先从现有 L2/L0 调用链画端到端 DAG，并把每个节点按 chunk 依赖分成两个顶层类别：

- **chunk 间无依赖**：当前 chunk 只依赖原始输入或已经就绪的局部中间量，可按 batch、head、chunk 或 tile 并行。
- **chunk 间有依赖**：当前 chunk 必须消费相邻或更早/更晚 chunk 的状态。对这类节点继续记录正向或反向传播方向、carry state、初始状态来源、更新点和 owner。

正向或反向是“chunk 间有依赖”节点的二级属性，不要与是否存在 chunk 依赖并列分类。layout、cast、copy 和边界适配也按其真实数据边归入上述两类，再根据代价和生命周期决定放入哪个 stage。

依赖边界首先决定内部 stage、执行顺序、状态所有权和同步协议，不自动等同于 L0 或 kernel 边界。声明为单 Kernel 大融合时，应满足：

1. 只有一个 kernel 入口和一套顶层资源、workspace、TPipe 与同步规划。
2. 正向递推、反向递推和 chunk 间无依赖计算作为 kernel 内部 stage，在各自依赖就绪的最早时刻执行。
3. 不在 kernel 内调用原有 L0 或嵌套启动其他 kernel；需要复用的数学过程下沉为无状态内部组件，由顶层 kernel 统一管理数据和生命周期。
4. 同一 carry 链由同一 owner 负责；确需跨 owner 时，显式设计 ready/free、状态位置和计数协议。
5. 融合只改变执行图和中间量生命周期，不改变既有输入、属性、元数据及其信息来源。
...
新增或修改 TilingKey、模板选择条件或 tiling 分支时，不能只提交“能够选到该 key”的输入 shape。先静态枚举 host tiling 中全部可达 key，再为每个 key 设计普通和边界 case，并用 host tiling UT 的 expected key 或运行时 tiling 日志证明实际命中。若一个 key 内仍有运行时分支，这些分支也要分别覆盖；不可达或明确不支持的组合应有校验或反向用例。完整记录格式见 [`验证方法与矩阵.md`](验证方法与矩阵.md)。
...
交付前先按 [`算子编码规范.md`](算子编码规范.md) 检查代码结构，再按 [`算子开发交付清单.md`](算子开发交付清单.md) 逐项核对；性能优化同时执行 [`../optimization/算子优化检查清单.md`](../optimization/算子优化检查清单.md)。尤其注意：
...
- 已有 ATK 工程时，以 ATK 精度作为每轮迭代门禁，并补齐 TilingKey 覆盖和实际命中证据。
