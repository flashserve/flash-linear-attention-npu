...
本清单用于性能设计和实现交付。通用工程交付同时执行 [`../development/算子开发交付清单.md`](../development/算子开发交付清单.md)。
...
- [ ] 实现结构符合 [`../development/算子编码规范.md`](../development/算子编码规范.md)，A5 路径同时符合 [`soc/A5优化约束.md`](soc/A5优化约束.md) 的编码规范。
...
- [ ] 全部可达 TilingKey 已映射到 ATK 普通 case、边界 case 和目标 SOC，同一 key 内的运行时分支也已覆盖。
- [ ] 每个 TilingKey 都有 host tiling UT 或运行时 tiling 记录作为实际命中证据，没有用输入 shape 代替命中验证。
- [ ] 所有 SOC 仍复用同一 L0 定义、原型和 L2 调用路径。

## 性能和验证

- [ ] 每轮只修改一个可测因素，并记录预期 bound；快速开发期间只强制 ATK 精度，精度通过后才把本次开发迭代计为完成。
- [ ] 准备验收该轮产物时已固定一个精度通过版本，并记录改进点、固定性能锚点的最终 Task Duration、相对基线变化、确定性和 sanitizer 结果。
- [ ] Release 编译和完整 ATK 精度矩阵通过。
