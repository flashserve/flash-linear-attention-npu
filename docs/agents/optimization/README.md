...
- `算子优化检查清单.md`：汇总优化设计与交付检查项。

## 阅读顺序

1. 先判断不同 chunk 是否共享递推 carry。
2. 无 carry 阅读 [`分块间无依赖算子`](dependency/分块间无依赖算子.md)，有 carry 阅读 [`分块间有依赖算子`](dependency/分块间有依赖算子.md)。
3. 所有优化任务阅读 [`执行模型与阶段设计`](techniques/执行模型与阶段设计.md)、[`内存规划与数据通路`](techniques/内存规划与数据通路.md) 和 [`流水与同步`](techniques/流水与同步.md)。
4. 涉及 tile、模板和物理布局时阅读 [`Tiling 与物理布局`](techniques/Tiling与物理布局.md)。
5. 开始性能试验和分析 profiling 前阅读 [`性能调优与瓶颈分析`](techniques/性能调优与瓶颈分析.md)。
6. 根据目标平台阅读 [`A2/A3 优化约束`](soc/A2-A3优化约束.md) 或 [`A5 优化约束`](soc/A5优化约束.md)。
7. 交付前执行 [`算子优化检查清单.md`](算子优化检查清单.md) 和 [`../development/验证方法与矩阵.md`](../development/验证方法与矩阵.md)。
