...
| 批次 | 算子 | 说明 |
|---|---|---|
| A（已完成试点） | `npu_recurrent_gated_delta_rule` | stable 已启用、parity/多 stream 通过 |
...
- **ND-only 可用**：输入都是普通 tensor/视图，无 5HD/NZ 风险 → 可进 stable；
- **参数形态复杂度**：
...
- **mutation**：`MUTATED_ARGUMENTS` 中列出的 in-place 参数需走 mutation 契约测试；
  ...
  [cpp-stable-launcher-measurement-report.md §8](cpp-stable-launcher-measurement-report.md)）；
  设备侧契约用例：`tests/stable_abi/regression_mutation_contract.py`；
...
|---|---|---|---|---|
| npu_recurrent_gated_delta_rule | A | 简单-中（optional g/gk） | state | **已 stable** |
...
| 算子 | JSON-only | 头校验 | parity | host（ctypes → stable public） |
|---|---|---|---|---|
...
| npu_solve_tri | ✅（bsnd/bnsd dense） | ✅ | 0.0（fp16/bf16 × BT 16/32/64/128）；`tnd` 因段错误被两个 wrapper 一致拒绝（记 SKIP）；**`ntd` 实测同样会段错误**（6 种拼写：5 段错误 + 1 个 161001，没有任何一种能出结果），但**故意不拦截**——参考实现也不拦，是否拦由算子责任人决定；早前"ntd 返回全 0"的注释是错的 | 910b：0.372 → 0.098 ms；950：0.033 → 0.009 ms（dense bsnd fp16 BT64） |
...
4. `npu_solve_tri`：bsnd/bnsd dense 已原生 stable 并 0.0；`tnd` 被拒绝（段错误）；
   **`ntd` 与 `tnd` 表现相同（段错误），但按"与参考实现保持一致"的要求不拦截**，
   ...
   `npu_chunk_gated_delta_rule_bwd` 的四个 spelling 在 950 的这份 OPP 上全是
   `161002`（tiling 未实现，两侧一致拒绝），已逐 spelling 记录。
5. 收尾：设备回归全绿（场景与运行方式见 `tests/stable_abi/README.md`）——
   ...
   chunk_gated_delta_rule_bwd）由 `regression_950_ops.py` 提供、`--group a5` 选中。

## 回退域收敛计划（目标：stable 覆盖 ctypes 的全部可用域）

当前仍会回退 ctypes 的点（已在代码里逐个核对）：

| 算子 | 回退条件 | 性质 | 结论/计划 |
| --- | --- | --- | --- |
| `npu_solve_tri` | `layout == "tnd"` | 上游 kernel 缺陷（段错误） | 明确拒绝（不是回退 ctypes）；`ntd` 同类缺陷但**不拦**（与参考一致），交算子责任人 |
