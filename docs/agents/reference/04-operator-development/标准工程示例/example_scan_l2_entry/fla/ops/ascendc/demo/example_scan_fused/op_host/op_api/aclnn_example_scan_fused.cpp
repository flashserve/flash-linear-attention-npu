/**
 * 示例文件：.../example_scan_fused/op_host/op_api/aclnn_example_scan_fused.cpp
 *
 * 注意事项：
 *   1. 组合入口的 L2 只做四件事：校验、按顺序调 L0、拼接公开输出、把 workspace 交给调用方。
 *      计算、分核、同步全部在被组合算子里，这里一行都不能重复实现。
 *   2. 依赖算子的 L0 头按相对路径 include（示例：`../../../example_scan/op_host/op_api/example_scan.h`）；
...
#include "aclnn_example_scan_fused.h"

#include "../../../example_scan/op_host/op_api/example_scan.h"
#include "../../../example_scan_tail/op_host/op_api/example_scan_tail.h"
...
constexpr int64_t EXAMPLE_SCAN_FUSED_SUPPORTED_DIM = 128;
constexpr int64_t EXAMPLE_SCAN_FUSED_SUPPORTED_CHUNK = 64;

struct ExampleScanFusedParams {
...
enum class ExampleScanFusedMode : int64_t { kInvalid = -1, kNone = 0, kSave = 1, kSaveTail = 2 };

ExampleScanFusedMode ResolveMode(const ExampleScanFusedParams &params)
{
    ...
    if (hasState != hasXNorm) {
        return ExampleScanFusedMode::kInvalid;
    }
    if (!hasState) {
        return hasTail ? ExampleScanFusedMode::kInvalid : ExampleScanFusedMode::kNone;
    }
    return hasTail ? ExampleScanFusedMode::kSaveTail : ExampleScanFusedMode::kSave;
}

aclnnStatus CheckParams(const ExampleScanFusedParams &params)
{
    CHECK_COND(params.x != nullptr && params.g != nullptr && params.yOut != nullptr,
               ACLNN_ERR_PARAM_NULLPTR, "x/g/yOut 不能为 nullptr。");
    const ExampleScanFusedMode mode = ResolveMode(params);
    CHECK_COND(mode != ExampleScanFusedMode::kInvalid, ACLNN_ERR_PARAM_INVALID,
               ...
               params.tailOut ? "非空" : "空");
    CHECK_COND(params.chunkSize == EXAMPLE_SCAN_FUSED_SUPPORTED_CHUNK, ACLNN_ERR_PARAM_INVALID,
               "本入口只支持 chunk_size=%ld，当前 chunk_size=%ld；其它取值请改用 example_scan。",
               static_cast<long>(EXAMPLE_SCAN_FUSED_SUPPORTED_CHUNK),
...
aclnnStatus aclnnExampleScanFusedGetWorkspaceSize(
...
{
    L2_DFX_PHASE_1(aclnnExampleScanFused,
    ...
    ExampleScanFusedParams params{x,          g,          aLogOptional,
    ...
    const auto scanOut = l0op::ExampleScan(
        ...
        mode == ExampleScanFusedMode::kNone ? 0 : 1, executorPtr);
    CHECK_RET(scanOut[0] != nullptr, ACLNN_ERR_INNER_NULLPTR);

    // 组合第二步：尾部归一化。只在 save+tail 档调用，中间结果用 executor 内部张量。
    if (mode == ExampleScanFusedMode::kSaveTail) {
        const auto tailOut = l0op::ExampleScanTail(scanOut[0], params.tailOut, executorPtr);
...
aclnnStatus aclnnExampleScanFused(void *workspace, uint64_t workspaceSize,
                                  aclOpExecutor *executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnExampleScanFused);
    CHECK_COND(CommonOpExecutorRun(workspace, workspaceSize, executor, stream) == ACLNN_SUCCESS,
               ACLNN_ERR_INNER, "ExampleScanFused AI Core 启动失败。");
    return ACLNN_SUCCESS;
}
