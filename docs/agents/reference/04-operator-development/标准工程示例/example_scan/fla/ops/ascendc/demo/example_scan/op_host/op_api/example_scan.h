/**
 * 示例文件：fla/ops/ascendc/demo/example_scan/op_host/op_api/example_scan.h
 *
 * 注意事项：
 ...
 *      保证 launcher 形参不发生压缩错位（见 example_scan.cpp 的注意事项）。
 *   4. outputMode 由 L2 传入，是编译期档位的选择依据；L0 不自己判断"调用方要不要"。
 *   5. 依赖的其它算子 L0（组合形态）通过 include 其 op_host/op_api/<依赖算子>.h 直接调用。
 */

#ifndef OP_API_INC_LEVEL0_EXAMPLE_SCAN_H
#define OP_API_INC_LEVEL0_EXAMPLE_SCAN_H
...
using ExampleScanOutputs = std::array<const aclTensor *, 3>;
...
#endif // OP_API_INC_LEVEL0_EXAMPLE_SCAN_H
