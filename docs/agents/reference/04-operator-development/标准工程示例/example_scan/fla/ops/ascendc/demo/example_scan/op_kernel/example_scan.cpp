/**
 * 示例文件：fla/ops/ascendc/demo/example_scan/op_kernel/example_scan.cpp
 *
 * 注意事项：
    ...
    GM_ADDR tiling)
{
    ...
    REGISTER_TILING_DEFAULT(ExampleScanTilingData);
    GET_TILING_DATA_WITH_STRUCT(ExampleScanTilingData, tilingData, tiling);

    ExampleScanArgs args{};
    args.x = x;
    args.g = g;
    args.aLog = a_log;
    ...
    using Policy = ExampleScanPolicy<
        static_cast<ExampleScanNormMode>(NORM_MODE), USE_STATE,
        static_cast<ExampleScanOutputMode>(OUTPUT_MODE)>;
    using XT = typename ExampleScanStorageType<D_T_X>::type;
    using GT = typename ExampleScanStorageType<D_T_G>::type;
    ExampleScanNs::RunExampleScan<XT, GT, Policy>(args);
}
