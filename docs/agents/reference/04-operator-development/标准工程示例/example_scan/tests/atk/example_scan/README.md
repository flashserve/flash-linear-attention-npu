<!--
示例文件：tests/atk/example_scan/README.md
...
# ExampleScan ATK 工程（示例骨架）

## 输入限制

与算子 [`README.md`](../../../fla/ops/ascendc/demo/example_scan/README.md) 保持一致：
...
`-- NPU DUT（fla_npu.ops.ascendc.example_scan）
```
...
| 文件 | 必须写什么 |
| --- | --- |
| `atk_example_scan.json` | 逻辑分支覆盖用例：每个 layout、每个 dtype 组合、chunk 64/128、fixed/varlen、tail/partial、`output_mode` 两档、负向用例（非法 chunk_size、D≠128、只给一个可选输出） |
| `atk_example_scan_perf.json` | 用户模型 case，保留原始 shape/dtype/属性；只跑 NPU，不做精度对比 |
| `atk_example_scan_mss.json` | 按全部可达 TilingKey 构造；每个 key 至少一条，覆盖 slot 复用与 Save 档搬出路径 |
| `example_scan.yaml` | ATK 生成配置，shape/dtype 必须满足算子 README 与 tiling 校验 |
