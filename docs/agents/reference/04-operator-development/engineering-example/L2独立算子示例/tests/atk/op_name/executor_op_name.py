...
def _op_name_ref(inputs: dict[str, Any]) -> tuple:
...
def run_cpu(spec: dict[str, Any], high_precision: bool = False) -> tuple:
    """CPU 高精度 golden。"""
    return _op_name_ref(build_inputs(spec, torch.device("cpu"), high_precision=True))
