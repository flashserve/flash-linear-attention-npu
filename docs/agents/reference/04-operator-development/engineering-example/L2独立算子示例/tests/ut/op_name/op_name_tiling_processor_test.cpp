...
using optiling::OpNameOutputMode;
using optiling::OpNameSchedule;
using optiling::OpNameScheduleContext;
using optiling::OpNameTilingProcessor;
...
void TestNoneModeAllocatesNoStateSlot()
{
    ...
    context.outputMode = OpNameOutputMode::OP_NAME_OUTPUT_MODE_NONE;

    OpNameSchedule schedule;
    Expect(OpNameTilingProcessor(context).Process(schedule), "none 档调度应成功");
...
void TestSaveModeAllocatesStateSlot()
{
    ...
    context.coreNum = 20;
    context.libApiWorkspaceBytes = 4096;
    context.outputMode = OpNameOutputMode::OP_NAME_OUTPUT_MODE_SAVE;

    OpNameSchedule schedule;
    Expect(OpNameTilingProcessor(context).Process(schedule), "save 档调度应成功");
...
void TestRejectsEmptyWork()
{
    ...
    context.coreNum = 20;
    OpNameSchedule schedule;
    Expect(!OpNameTilingProcessor(context).Process(schedule),
           "head 数为 0 时必须返回失败而不是给出默认调度");
}

void TestMaskMappingMatchesMode()
{
    Expect(optiling::OP_NAME_NONE_OUTPUT_MASK == optiling::OP_NAME_REQUIRED_OUTPUT_MASK,
           "none 档只能包含必选输出");
    Expect((optiling::OP_NAME_SAVE_OUTPUT_MASK & optiling::OP_NAME_OPTIONAL_OUTPUT_MASK) ==
               optiling::OP_NAME_OPTIONAL_OUTPUT_MASK,
           "save 档必须包含全部可选输出");
}
