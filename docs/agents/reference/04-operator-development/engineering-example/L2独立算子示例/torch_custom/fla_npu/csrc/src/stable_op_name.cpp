/**
 * 示例文件：torch_custom/fla_npu/csrc/src/stable_op_name.cpp
 *
 * 注意事项：
 *   1. 一算子一个文件：文件名 = 算子名去掉 npu_ 前缀（npu_op_name -> stable_op_name.cpp）。
 *      新建后必须在 stable_ops.cpp 加 `#include "stable_op_name.cpp"` 一行 + 注册两行，
...
constexpr const char* kOpNameLayoutNames[] = {"BSND", "BNSD", "TND", "NTD"};

constexpr const char* kSchema_op_name =
    "npu_op_name(Tensor x, Tensor g, Tensor? a_log, Tensor? initial_state, "
...
std::tuple<Tensor, std::optional<Tensor>, std::optional<Tensor>> run_npu_op_name(
  ...
  FLA_STABLE_EXEC("aclnnOpName", x_meta, stream, tensor(x_meta),
                  ...
                  cstr(kOpNameLayoutNames, layout), scalar(scale), scalar(chunk_size),
