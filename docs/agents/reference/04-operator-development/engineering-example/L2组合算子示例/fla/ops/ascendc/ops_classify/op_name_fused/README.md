# OpNameFused 示例算子说明（形态 C：只有 L2）

<!--
示例文件：fla/ops/ascendc/ops_classify/op_name_fused/README.md
...
```text
OpNameFused(x, g, ...) =
    OpName        （主实现，产出 y/state/x_norm）
      -> OpNameTail（尾部归一化，产出 tail）
```
...
| --- | --- | --- |
| `x`、`g` | 必选 | 与 `OpName` 相同 |
| `a_log`、`initial_state`、`cu_seqlens`、`chunk_indices` | 可选 | 直接透传给 `OpName` |
| `layout`、`scale`、`chunk_size`、`epsilon` | 属性 | 与 `OpName` 相同 |
| `y` | 必选输出 | 来自 `OpName` |
| `state`、`x_norm` | 可选输出 | 来自 `OpName`；同时给出或同时为空 |
| `tail` | 可选输出 | 来自 `OpNameTail`，需要尾部归一化时给出 |

## 已知限制

1. `x` 必须为 BF16、`D=128`、`chunk_size=64`、公开输出连续；其它组合返回
   `ACLNN_ERR_PARAM_INVALID` 并提示改用 `op_name` 单算子入口。
