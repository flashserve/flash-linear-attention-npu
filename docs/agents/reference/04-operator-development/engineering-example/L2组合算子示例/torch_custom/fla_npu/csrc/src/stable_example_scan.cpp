/**
 * 示例文件（形态 B）：torch_custom/fla_npu/csrc/src/stable_example_scan.cpp
 *
 * 场景：该算子已发布 V1（`aclnnExampleScan`），现在要新增 V2（`aclnnExampleScanV2`）。
 * 本文件是 `stable_example_scan.cpp` 的**增量**：schema 不变，只在 `run_` 里加一条分支。
 *
 * 注意事项：
 *   1. 一个 schema ↔ 一个 `run_`：V1/V2 都由同一个 `kSchema_example_scan` 进入，
 ...
 *      `aclnn_example_scan_v2.h` 头文件注释、`docs/api.md` 的支持范围逐条一致。
...
// ... 文件头部：schema（不变）与 run_ 签名（不变） ...
  ...
  // 场景选择：命中 V2 的组合场景走 aclnnExampleScanV2，其余回落签名未变的 aclnnExampleScan。
  const bool useV2 = example_scan_supports_v2(x_meta, layout, chunk_size, tail_mode);
  if (useV2) {
    FLA_STABLE_EXEC("aclnnExampleScanV2", x_meta, stream, tensor(x_meta),
                    tensor(meta_of(g)), optional_tensor(a_log),
                    optional_tensor(initial_state), int_array(cu_seqlens),
                    int_array(chunk_indices), cstr(kExampleScanLayoutNames, layout),
