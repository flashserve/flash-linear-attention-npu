 ...
 *      `stable_op_name.cpp`（形态 B：在已有适配里加 V2 分支）→ `stable_op_name_fused.cpp`
 *      （形态 C：新的组合算子）。
 *   2. 形态 B 不新增 schema：V1/V2 是同一个 `npu_op_name` 的两条 aclnn 入口，由 `run_` 内部选择
 *      （参考 `stable_chunk_kda_fwd.cpp` 对 `aclnnChunkKdaFwdV2` / `aclnnChunkKdaFwd` 的分支），
 *      所以这里**不改注册**。
 *   3. 形态 C 新增一个算子：`m.def(kSchema_op_name_fused)` +
 *      `m.impl("npu_op_name_fused", &boxed_adapter<run_npu_op_name_fused>)` 两行都要加。
...
#include "stable_op_name.cpp"        // 形态 B：同一算子加了 V2 分支
#include "stable_op_name_fused.cpp"  // 形态 C：新的组合算子
// ... 其余算子按算子名排序 ...

STABLE_TORCH_LIBRARY(fla_npu_stable, m) {
  // ... 既有 m.def(...) ...
  // 形态 B 不新增 schema；形态 C 新增一行：
  m.def(kSchema_op_name_fused);
}

STABLE_TORCH_LIBRARY_IMPL(fla_npu_stable, CompositeExplicitAutograd, m) {
  // ... 既有 m.impl(...) ...
  m.impl("npu_op_name_fused",
         &fla_npu_stable::stable::boxed_adapter<run_npu_op_name_fused>);
}
