/**
 * 示例文件（形态 C）：torch_custom/fla_npu/csrc/src/stable_op_name_fused.cpp
 *
 * 交付布局依据 torch_custom/fla_npu/README.md §1.1：组合算子没有 def/kernel，但对外仍然是一个
 * 算子入口，所以交付件与普通算子完全相同（4 个文件）：
 *
 *   csrc/src/stable_op_name_fused.cpp   # 本文件（新建，文件名 = 算子名去掉 npu_）
 ...
 *
 * 注意事项：
 *   1. 一算子一文件、用宏写：`kSchema_op_name_fused` 形参 === `run_` 形参 ===
...
// Stable-ABI adapter for npu_op_name_fused.
// aclnn: aclnnOpNameFused
...
// 名表顺序必须与 _stable._ENUM["npu_op_name_fused"]["layout"] 一致。
constexpr const char* kOpNameFusedLayoutNames[] = {"BSND", "BNSD", "TND", "NTD"};

constexpr const char* kSchema_op_name_fused =
    "npu_op_name_fused(Tensor x, Tensor g, Tensor? a_log, Tensor? initial_state, "
...
run_npu_op_name_fused(Tensor x, Tensor g, std::optional<Tensor> a_log,
