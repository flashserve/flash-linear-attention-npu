...
>
> **两个占位符**：路径里的 `ops_classify` 是**算子类别段**（实际交付时替换为算子在 `fla/ops/ascendc/`
> 下的真实类别，例如 `gdn`、`kda`），`op_name` 是**算子名占位符**（替换为真实算子名）。替换时目录名、
...
1. 复制 `L2独立算子示例/fla/ops/ascendc/ops_classify/op_name/` 到目标位置，把类别段 `ops_classify` 换成
   该算子所属类别（`fla/ops/ascendc/` 下的真实类别名，如 `gdn`、`kda`），把 `op_name` 换成真实算子名；
