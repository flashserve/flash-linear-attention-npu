/**
 * 示例文件：fla/ops/ascendc/demo/example_scan/op_host/example_scan_tiling.cpp
 *
 * 注意事项：
...
#include "example_scan_tiling.h"

#include <cstdint>
#include <cstring>
#include <limits>

#include "../op_kernel/example_scan_tiling_key.h"
#include "arch22/example_scan_tiling_impl.h"
#include "arch35/example_scan_tiling_impl.h"
...
constexpr int64_t EXAMPLE_SCAN_SUPPORTED_DIM = 128;
constexpr uint32_t EXAMPLE_SCAN_MIX_BATCH_MODE = 1;

// host 档位枚举（example_scan_output_mask.h）与 kernel 侧 TPL token（example_scan_tiling_key.h）
// 必须是同一组数值，否则 L2 算出的档位与 kernel 实例不匹配。
static_assert(EXAMPLE_SCAN_OUTPUT_MODE_NONE == ExampleScanNs::EXAMPLE_SCAN_TPL_OUTPUT_NONE,
              "host 与 kernel 的 none 档位取值必须一致。");
static_assert(EXAMPLE_SCAN_OUTPUT_MODE_SAVE == ExampleScanNs::EXAMPLE_SCAN_TPL_OUTPUT_SAVE,
              "host 与 kernel 的 save 档位取值必须一致。");

struct ExampleScanShapeInfo {
    int64_t batch = 0;
    int64_t seqLen = 0;
    int64_t headNum = 0;
    int64_t dim = 0;
...
bool ResolveShape(gert::TilingContext *context, ExampleScanShapeInfo &info)
{
    const auto xShape = context->GetInputShape(EXAMPLE_SCAN_INPUT_X);
    if (xShape == nullptr) {
        return false;
    }
...
bool CheckShapeAndAttrs(gert::TilingContext *context, const ExampleScanShapeInfo &info,
                        float &scale, float &epsilon, uint32_t &chunkSize, uint32_t &outputMode)
{
    ...
    const float *scalePtr = attrs->GetAttrPointer<float>(EXAMPLE_SCAN_ATTR_SCALE);
    const int64_t *chunkSizePtr = attrs->GetAttrPointer<int64_t>(EXAMPLE_SCAN_ATTR_CHUNK_SIZE);
    const float *epsilonPtr = attrs->GetAttrPointer<float>(EXAMPLE_SCAN_ATTR_EPSILON);
    const int64_t *outputModePtr = attrs->GetAttrPointer<int64_t>(EXAMPLE_SCAN_ATTR_OUTPUT_MODE);
    ...
    if (info.dim != EXAMPLE_SCAN_SUPPORTED_DIM) {
        OP_LOGE(context->GetNodeName(), "D 只支持 %ld，当前 Ddim=%ld。",
                static_cast<long>(EXAMPLE_SCAN_SUPPORTED_DIM), static_cast<long>(info.dim));
        return false;
    }
    ...
    if (*outputModePtr != EXAMPLE_SCAN_OUTPUT_MODE_NONE &&
        *outputModePtr != EXAMPLE_SCAN_OUTPUT_MODE_SAVE) {
...
bool DtypeToTemplateToken(ge::DataType dtype, uint64_t &token)
{
    switch (dtype) {
        case ge::DT_BF16:
            token = ExampleScanNs::EXAMPLE_SCAN_TPL_BF16;
            return true;
        case ge::DT_FLOAT16:
            token = ExampleScanNs::EXAMPLE_SCAN_TPL_FP16;
            return true;
        case ge::DT_FLOAT:
            token = ExampleScanNs::EXAMPLE_SCAN_TPL_FP32;
            return true;
        default:
            return false;
    }
...
ge::graphStatus Tiling4ExampleScan(gert::TilingContext *context)
{
    const auto attrs = context->GetAttrs();
    if (attrs == nullptr) {
        OP_LOGE(context->GetNodeName(), "属性表为空。");
    ...
    const char *layout = attrs->GetAttrPointer<char>(EXAMPLE_SCAN_ATTR_LAYOUT);

    ExampleScanShapeInfo shape;
    ...
    const auto xDesc = context->GetInputDesc(EXAMPLE_SCAN_INPUT_X);
    const auto gDesc = context->GetInputDesc(EXAMPLE_SCAN_INPUT_G);
    const auto stateDesc = context->GetOptionalInputDesc(EXAMPLE_SCAN_INPUT_INITIAL_STATE);
    ...
    const uint32_t archTileT =
        isAscend950 ? EXAMPLE_SCAN_ARCH35_TILE_T : EXAMPLE_SCAN_ARCH22_TILE_T;

    ExampleScanScheduleContext scheduleContext;
    ...
    if (!ExampleScanTilingProcessor(scheduleContext).Process(schedule)) {
        OP_LOGE(context->GetNodeName(),
    ...
    // 模板参数顺序必须与 op_kernel/example_scan_tiling_key.h 的 ASCENDC_TPL_ARGS_DECL 一致。
    const uint64_t tilingKey = GET_TPL_TILING_KEY(
        xToken, gToken, ExampleScanNs::EXAMPLE_SCAN_TPL_NORM_L2,
    ...
    if (context->SetScheduleMode(EXAMPLE_SCAN_MIX_BATCH_MODE) != ge::GRAPH_SUCCESS) {
    ...
    OP_LOGI(context->GetNodeName(),
            "ExampleScan layout=%s tilingKey=%lu usedCoreNum=%u headsPerCore=%u outputMode=%u "
...
ge::graphStatus TilingPrepareForExampleScan(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ExampleScan)
    .Tiling(Tiling4ExampleScan)
    .TilingParse<ExampleScanCompileInfo>(TilingPrepareForExampleScan);
