...
`ops=` 使用 `tests/op_cases/*.json` 中的算子名，多个算子用英文逗号连接，评论命令中逗号后不要加空格。例如修改
`causal_conv1d` 和 `chunk_bwd_dv_local` 时，使用
`/run-npu-ci quick ops=causal_conv1d,chunk_bwd_dv_local`。`quick` 未指定 `ops=` 时不再串行运行全部算子的泛化和主精度矩阵，
只保留 quick 基础检查与 Example/ST；指定 `ops=` 时仅运行所列算子矩阵。`full` 未指定 `ops=` 时运行全部已注册算子。
