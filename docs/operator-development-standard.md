...
### 4.3 `op_api/aclnn` 编码规范

`op_api/aclnn` L2 必须承接完整的公共算子语义，并隐藏 L0 kernel 的内部 phase、临时类型转换和调度细节。新增或重要修改 Ascend C 算子时，设计与评审必须逐项说明：
...
- 使用 `SyncAll`/`TPipe::Reset()` 时，算子 `docs/design.md` 必须记录参与核类型和数量、schedule mode、A2/A3/A5 支持情况、同步前后的数据可见性、Reset 前已排空的队列和 Reset 后重新初始化的资源。不能用它们掩盖本可通过任务划分消除的依赖。

L2 与类型转换还必须遵守：

- L2 负责公共参数校验、layout/view 规范化、输出描述、workspace/executor 管理，以及按数据依赖在同一 stream 上完成 launch 编排；不得把内部 phase 选择权上抛给调用者。
