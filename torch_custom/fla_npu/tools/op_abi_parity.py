...
# (measured: ~100 KiB per recurrent call, 8.2 GiB before the conc32 service
# OOMed).  Optional slots use to<std::optional<Tensor>>, which consumes the inner
# handle and frees its box.  A *present* optional puts that box pointer in the
# slot, so reading it with to<Tensor> would wrap the pointer as an
# AtenTensorHandle and delete it as a tensor.  Neither pattern may appear as a
# raw handle read.
