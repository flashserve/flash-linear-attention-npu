...
def npu_chunk_kda_fwd_prepare(
    ...
    *,
    ...
    scale_fp32 = ctypes.c_float(scale).value
    epsilon_fp32 = ctypes.c_float(epsilon).value
    lower_bound_fp32 = ctypes.c_float(lower_bound).value
    if not math.isfinite(scale_fp32):
        raise RuntimeError(f"{op_name}: scale must be finite.")
    if not math.isfinite(epsilon_fp32) or epsilon_fp32 <= 0.0:
        raise RuntimeError(f"{op_name}: epsilon must be finite and greater than zero.")
    if not math.isfinite(lower_bound_fp32):
    ...
    if use_gate_in_kernel:
        ...
        if safe_gate and not (-5.0 <= lower_bound_fp32 < 0.0):
            raise RuntimeError(
