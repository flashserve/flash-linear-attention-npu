...
def npu_gdn_core_fwd_phase6(
    ...
    *,
    ...
    ``return_aux=False`` keeps the fixed four-slot return contract but returns
    ``None`` for ``g_cumsum`` and ``A``. The fused kernel still computes its
    internal cumsum and A dependencies while skipping their public writes.
    """
    import torch
    ...
    o = _empty_like(v)
    g_cumsum = _empty((batch, tokens, v_heads), g, dtype=torch.float32) if return_aux else None
    A = _empty((batch, v_heads, tokens, int(chunk_size)), q) if return_aux else None
    final_state = None
    if output_final_state:
